...
define(['jquery', 'core/config', 'core/templates'], function ($, mdlconfig, templates) {
   var report_table = {
        ...
        change_links: function (uniqueid, blockid) {
            ...
            $('.block_report_table.' + uniqueid + ' .content .rb-display-table-container').
                ...
                $(this).click(function(e){
                    ...
                    if ($(e.target).siblings('.instantreportwait').length === 0) {
                        var $splash = $('<div class="instantreportwait"></div>'),
                            $content = $('.block_report_table.' + uniqueid + ' .content .rb-display-table-container');

                        $content.addClass('loading');
                        $content.append($splash);

                        $('.reportbuilder-table').html('<img src="' + M.cfg.wwwroot +'/pix/y/loading.gif"/>');
                    }
        ...
        refresh_results: function (uniqueid, querystring, callback) {
            // Make the ajax call.
            return $.get(
                mdlconfig.wwwroot + '/blocks/report_table/ajax_instantreport.php?' + querystring
            ).done( function (data) {
                var $content = $('.block_report_table.' + uniqueid + ' .content');
        ...
        populatelist: function() {
            ...
            $('#id_config_reportid').change(function () {
                ...
                M.util.js_pending('block_report_table-populatelist');

                // Make an AJAX request to get the saved searches.
                $.ajax({
                    type: 'POST',
                    url: mdlconfig.wwwroot + '/blocks/report_table/ajax_list_saved.php',
                    data: {
                        'reportid': reportId,
                    ...
                    success: function(data) {
                        ...
                        savedListControl.append('<option value="">' +
                            M.util.get_string('allavailabledata', 'block_report_table') + '</option>');
                        ...
                        M.util.js_complete('block_report_table-populatelist');
                    }
                });
            });
        }
