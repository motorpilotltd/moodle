 ...
 * @package block_report_table
 */

defined('MOODLE_INTERNAL') || die();

/**
 * T0tara report table block.
 *
 ...
 * @package block_report_table
 */
class block_report_table extends block_base {
    ...
    public function init() {
        $this->title = get_string('pluginname', 'block_report_table');
    }
    ...
    public function get_required_javascript() {
        ...
        $this->page->requires->js_call_amd('block_report_table/module', 'change_links', array($this->get_uniqueid(), $this->instance->id));
    }
    ...
    protected function get_uniqueid() {
        return 'block_report_table_' . $this->instance->id;
    }
    ...
    public function get_content() {
        ...
        if (isset($SESSION->nocapsblockreporttable[$id])) {
            if ($SESSION->nocapsblockreporttable[$id] > time()) {
                return $this->content;
            } else {
                unset($SESSION->nocapsblockreporttable[$id]);
            }
        }
        ...
        if (!reportbuilder::is_capable($id)) {
            // Performance: Temporarily turn off block in session for some time if no caps.
            if (empty($SESSION->nocapsblockreporttable) || !is_array($SESSION->nocapsblockreporttable)) {
                $SESSION->nocapsblockreporttable = array();
            }
            $SESSION->nocapsblockreporttable[$id] = time() + 300;
            return $this->content;
        }
        ...
        if ($report->is_initially_hidden()) {
            ...
            $reportlink = html_writer::link($reporturl, get_string('gotoreportpage', 'block_report_table'));
            $message = get_string('reportinitialdisplayfiltered', 'block_report_table', ['reportname' => $reportname, 'reportlink' => $reportlink]);
        ...
        $renderer = $this->page->get_renderer('local_reportbuilder');
        ...
        if ($report->has_disabled_filters()) {
            global $OUTPUT;
            $reporthtml = $OUTPUT->notification(get_string('filterdisabledwarning', 'local_reportbuilder'), 'warning') . $reporthtml;
        }
