...
class block_report_table_edit_form extends block_edit_form {
    /**
    ...
    protected function specific_definition($mform) {
        ...
        $PAGE->requires->string_for_js('allavailabledata', 'block_report_table');
        $PAGE->requires->js_call_amd('block_report_table/module', 'populatelist', array());

        // Output the form.
        $mform->addElement('header', 'configheader', get_string('blocksettings', 'block'));

        // Block title.
        $mform->addElement('text', 'config_title', get_string('blocktitle', 'block_report_table'));
        $mform->addHelpButton('config_title', 'blocktitle', 'block_report_table');
        ...
        if (isset($this->block->config->reportid) && !isset($reportoptions[$this->block->config->reportid])) {
            $reportoptions[$this->block->config->reportid] = get_string('inaccessiblereport', 'block_report_table');
        }

        $mform->addElement('select', 'config_reportid', get_string('report', 'local_reportbuilder'), $reportoptions);
        ...
        $savedoptions = array('0' => get_string('allavailabledata', 'block_report_table'));
        ...
        if ($reportid && isset($allowed[$reportid])) {
            ...
            if (isset($this->block->config->savedsearch) && !isset($savedoptions[$this->block->config->savedsearch])) {
                $savedoptions[$this->block->config->savedsearch] = get_string('inaccessiblesavedsearch','block_report_table');
            }
        }

        // Saved search.
        $mform->addElement('select', 'config_savedsearch', get_string('savedsearch', 'block_report_table'),
                $savedoptions);
        $mform->addElement('static', 'savedsearchdesc', '', get_string('savedsearchpublic', 'block_report_table'));
