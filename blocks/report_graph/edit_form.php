...
class block_report_graph_edit_form extends block_edit_form {
    ...
    protected function specific_definition($mform) {
        ...
        require_once($CFG->dirroot . '/blocks/report_graph/block_report_graph.php');

        $mform->addElement('header', 'configheader', get_string('blocksettings', 'block'));

        $mform->addElement('text', 'config_title', get_string('title', 'block_report_graph'));
        $mform->setType('config_title', PARAM_TEXT);

        $prevreport = false;
        if (!empty($this->block->config->reportorsavedid)) {
            $prevreport = \block_report_graph\util::get_report($this->block->config->reportorsavedid);
        }
        ...
        $options = array();
        $options[get_string('reports', 'local_reportbuilder')] = $reportoptions;
        if ($savedoptions) {
            $options[get_string('savedsearches', 'local_reportbuilder')] = $savedoptions;
        }

        $mform->addElement('selectgroups', 'config_reportorsavedid', get_string('report', 'local_reportbuilder'), $options);

        // View report data as user selection.

        $guest = guest_user();
        $options = array(
            $USER->id => get_string('reportforme', 'block_report_graph', fullname($USER)),
            0 => get_string('reportforcurrent', 'block_report_graph'),
            $guest->id => get_string('reportforguest', 'block_report_graph'),
        );
        ...
        if ($prevreport and isset($this->block->config->reportfor)) {
            ...
            if (!isset($options[$this->block->config->reportfor])) {
                if ($user = $DB->get_record('user', array('id' => $this->block->config->reportfor, 'deleted' => 0))) {
                    $options[$user->id] = get_string('reportforother', 'block_report_graph', fullname($user));
                }
            }
        }

        $mform->addElement('select', 'config_reportfor', get_string('reportfor', 'block_report_graph'), $options);
        $mform->addHelpButton('config_reportfor', 'reportfor', 'block_report_graph');
        ...
        $mform->addElement('select', 'config_cachettl', get_string('cachettl', 'block_report_graph'), $options);
        $mform->setDefault('config_cachettl', 60*60*1);

        $mform->addElement('header', 'graphappearance', get_string('graphappearance', 'block_report_graph'));

        $mform->addElement('text', 'config_graphimage_maxheight', get_string('maxheight', 'block_report_graph'));
        $mform->setType('config_graphimage_maxheight', PARAM_TEXT);
        $mform->addHelpButton('config_graphimage_maxheight', 'maxheight', 'block_report_graph');

        $mform->addElement('text', 'config_graphimage_maxwidth', get_string('maxwidth', 'block_report_graph'));
    ...
    function validation($data, $files) {
        ...
        $cache = cache::make('block_report_graph', 'graph');
        if (!empty($this->block->config->reportorsavedid) and isset($this->block->config->reportfor)) {
            $key = \block_report_graph\util::get_cache_key($this->block->config->reportorsavedid, $this->block->config->reportfor);
            $cache->delete($key);
        }
        if (!empty($data['config_reportorsavedid'])) {
            $key = \block_report_graph\util::get_cache_key($data['config_reportorsavedid'], $data['config_reportfor']);
            $cache->delete($key);
        }
        ...
        if ($data['config_reportorsavedid'] > 0) {
            ...
            if (isset($this->myreports[$reportid])) {
                ...
                if (!$prevreport or $prevreport->id != $reportid) {
                    if (!isset($this->myusers[$reportfor])) {
                        ...
                        $errors['config_reportorsavedid'] = get_string('errorconfigreportfor', 'block_report_graph');
                        return $errors;
                    }
                }
            } else {
                ...
                if (!$prevreport or $prevreport->id != $reportid) {
                    $errors['config_reportorsavedid'] = get_string('errorconfigreport', 'block_report_graph');
                    return $errors;
                } else {
                    ...
                    if ($prevreportfor != $reportfor) {
                        ...
                        $errors['config_reportfor'] = get_string('errorconfigreportfor', 'block_report_graph');
                        return $errors;
                    }
                }
            }
            ...
            $newreport = \block_report_graph\util::get_report(-$savedid);
        ...
        if (!empty($data['config_graphimage_maxwidth'])) {
            $result = \block_report_graph\util::normalise_size_and_user_input($data['config_graphimage_maxwidth']);
            if ($result === null) {
                // Its not a valid max width.
                $errors['config_graphimage_maxwidth'] = get_string('errormaxwidthinvalid', 'block_report_graph');
            }
        }

        // Validate maxwidth and maxheight are in an acceptable format.
        if (!empty($data['config_graphimage_maxheight'])) {
            $result = \block_report_graph\util::normalise_size_and_user_input($data['config_graphimage_maxheight']);
            if ($result === null) {
                // Its not a valid max width.
                $errors['config_graphimage_maxheight'] = get_string('errormaxheightinvalid', 'block_report_graph');
            }
        }
