...
class block_report_graph extends block_base {

    /**
     * A record from the database generated by \block_report_graph\util::get_report
     * @var stdClass
     */
    protected $rawreport;
    ...
    public function init() {
        $this->title = get_string('pluginname', 'block_report_graph');
    }
    ...
    public function specialization() {
        if (!empty($this->config->title)) {
            ...
            if ($this->is_configured()) {
                ...
                if (!isset($this->rawreport)) {
                    $this->rawreport = \block_report_graph\util::get_report($this->config->reportorsavedid);
                }
                if ($this->rawreport) {
        ...
        if (!empty($this->config->graphimage_maxwidth)) {
            $this->graphimage_maxwidth = \block_report_graph\util::normalise_size_and_user_input($this->config->graphimage_maxwidth);
        }
        if (!empty($this->config->graphimage_maxheight)) {
            $this->graphimage_maxheight = \block_report_graph\util::normalise_size_and_user_input($this->config->graphimage_maxheight);
        }
    }
    ...
    public function get_content() {
        ...
        if (empty($this->rawreport->type)) {
            ...
            $url = new moodle_url('/local/reportbuilder/graph.php', array('reportid' => $this->rawreport->id));
            $this->content->text = '<div><a href="'.$url.'">'.get_string('errornograph', 'block_report_graph').'</a></div>';
        ...
        if (reportbuilder::is_capable($this->rawreport->id, $USER->id)) {
            $url = new moodle_url('/local/reportbuilder/report.php', array('id' => $this->rawreport->id));
            if (!empty($this->rawreport->savedid)) {
                $url->param('sid', $this->rawreport->savedid);
            }
            $this->content->footer = '<a href="'.$url.'">'.get_string('report', 'local_reportbuilder').'</a>';
        }
        ...
        if (core_useragent::check_browser_version('MSIE', '6.0') and !core_useragent::check_browser_version('MSIE', '9.0')) {
            ...
            $svgurl = new moodle_url('/blocks/report_graph/ajax_graph.php', array('blockid' => $this->instance->id, 'type' => 'pdf'));
            ...
            $svgurl = new moodle_url('/blocks/report_graph/ajax_graph.php', array('blockid' => $this->instance->id, 'type' => 'svg'));
            $nosvg = get_string('error:nosvg', 'local_reportbuilder');
