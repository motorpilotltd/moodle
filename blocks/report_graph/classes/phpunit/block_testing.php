...
namespace block_report_graph\phpunit;
...
trait block_testing {
    ...
    protected function create_report_graph_block_instance($rid, array $config = []) {
        ...
        $page->blocks->add_block('report_graph', BLOCK_POS_LEFT, 0, false, '*', null);

        $block = $DB->get_record('block_instances', ['blockname' => 'report_graph'], '*', IGNORE_MULTIPLE);
        $DB->set_field('block_instances', 'configdata', $configdata, ['id' => $block->id]);
        $block->configdata = $configdata;

        return block_instance('report_graph', $block);
    }
}