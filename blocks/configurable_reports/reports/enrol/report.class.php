...
class report_enrol extends report_base {
        ...
        if($rs) {
            foreach ($rs as $rsrow) {
                if (!isset($rows[$rsrow->cid])) {
                    ...
                    $rows[$rsrow->cid]->enrolments = array();
                ...
                if (!isset($rows[$rsrow->cid]->enrolments[$rsrow->eenrol])) {
                    $rows[$rsrow->cid]->enrolments[$rsrow->eenrol] = get_string('pluginname', 'enrol_'.$rsrow->eenrol);
                }
            }
            ...
            foreach ($rows as $row) {
                $enrolments = '';
                asort($row->enrolments);
                foreach ($row->enrolments as $enrolment => $name) {
                    $enrolments .= $name
                        . '<br />';
                }
                $row->enrolments = substr($enrolments, 0, -6);
            }
        }
