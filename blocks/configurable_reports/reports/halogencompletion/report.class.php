...
class report_halogencompletion extends report_base {
    ...
    public function create_report() {
        ...
        $where = " AND u.idnumber != '' AND comp.timecompleted > 0";
        $where .= " AND comp.timecompleted > :monthsago3";
        if ($COURSE->id != SITEID) {
        ...
        $rs = $DB->get_recordset_sql($sql, array('monthsago3' => strtotime('midnight 3 months ago')));
        $rows = array();
        if($rs) {
            foreach ($rs as $rsrow) {
    ...
    public function print_filters() {
        ...
        echo html_writer::tag('p', 'This report will only show completions since '.date('d M Y', $monthsago3).', regardless of any earlier settings in filter choices.');
