...
/* BEGIN CORE MOD */
    if(has_capability('block/configurable_reports:managereports', $context, $userid)){
        if($courseid == SITEID && $allcourses) {
            $reports = $DB->get_records_select('block_configurable_reports', '(courseid = ? OR courseid = ?) ORDER BY name ASC', array($courseid, SITEID));
        } else {
            $reports = $DB->get_records_select('block_configurable_reports','(courseid = ? OR courseid = ?) ORDER BY name ASC', array($courseid, SITEID));
        }
    } else {
        $reports = $DB->get_records_select('block_configurable_reports', 'ownerid = ? AND (courseid = ? OR courseid = ?) ORDER BY name ASC', array($userid, $courseid, SITEID));
    }
/* END CORE MOD */
    return $reports;
}
