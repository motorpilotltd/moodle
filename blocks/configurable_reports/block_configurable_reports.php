...
/* BEGIN CORE MOD */
    private function _get_content_new() {
        global $DB, $USER, $CFG, $COURSE;
        ...
        if (!empty($this->config->displayglobalreports)) {
            ...
            foreach ($reports as $report) {
                if ($report->visible && cr_check_report_permissions($report, $USER->id, $context)) {
                    $bits = explode(' - ', $report->name);
                    $prefix =  (count($bits) === 2) ? format_string(ucfirst(strtolower($bits[0]))) : 'ZZZZZZ';
                    if (!isset($content->global[$prefix])) {
                        $content->global[$prefix] = array();
                    }
                    $rname = (count($bits) === 2) ? format_string($bits[1]) : format_string($report->name);
                    $rurl = new \moodle_url('/blocks/configurable_reports/viewreport.php', array('id' => $report->id, 'courseid' => $course->id));
                    $content->global[$prefix][] = \html_writer::link($rurl, $rname, array('alt' => $rname, 'title' => $rname, 'class' => 'accordion-item'));
                }
            }
        }
            ...
            if ($reports) {
                foreach ($reports as $report) {
                    if (!$report->global && $report->visible && cr_check_report_permissions($report, $USER->id, $context)) {
                        $bits = explode(' - ', $report->name);
                        $prefix =  (count($bits) === 2) ? format_string(ucfirst(strtolower($bits[0]))) : 'ZZZZZZ';
                        if (!isset($content->course[$prefix])) {
                            $content->course[$prefix] = array();
                        }
                        $rname = (count($bits) === 2) ? format_string($bits[1]) : format_string($report->name);
                        $rurl = new \moodle_url('/blocks/configurable_reports/viewreport.php', array('id' => $report->id, 'courseid' => $course->id));
                        $content->course[$prefix][] = \html_writer::link($rurl, $rname, array('alt' => $rname, 'title' => $rname, 'class' => 'accordion-item'));
                    }
                }
            }
        }

        if (!empty($content->global) || !empty($content->course)) {
            $renderer = $this->page->get_renderer('block_configurable_reports');
            $this->content->text = $renderer->render_content($content);

            // JS
            $js = <<<EOJ
(function ( $ ) {
    $('.block_configurable_reports .accordion .collapse').on('show', function () {
        $(this).parents('.accordion-group').siblings('.accordion-group').find('a.accordion-toggle').addClass('collapsed');
    });
}(jQuery));
EOJ;
