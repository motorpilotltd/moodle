...
/* BEGIN CORE MOD */
        $starttimestamp = make_timestamp(date('Y'), 4, 1, 0, 0, 0);
        if ($starttimestamp > time()) {
            $starttimestamp = make_timestamp(date('Y') - 1, 4, 1, 0, 0, 0);
        }
        $mform->addElement('date_time_selector', 'filter_starttime', get_string('starttime', 'block_configurable_reports'));
        $mform->setDefault('filter_starttime', $starttimestamp);
        $mform->addElement('date_time_selector', 'filter_endtime', get_string('endtime', 'block_configurable_reports'));
        $mform->setDefault('filter_endtime', make_timestamp(date('Y'), date('m'), date('d'),  23, 59, 59));
/* END CORE MOD */
    }
}
