...
/* BEGIN CORE MOD */
                case 'name':
                case 'id':
                    $row->{$data->column} = ($row->{$data->column}) ? \html_writer::link(new \moodle_url('/course/category.php', array('id' => $row->id)), $row->{$data->column}) : '--';
                    break;
                case 'timemodified':
                    $row->{$data->column} = ($row->{$data->column})? userdate($row->{$data->column}, '%d %b %Y, %H:%M'): '--';
/* END CORE MOD */
                    break;
                case 'visible':
