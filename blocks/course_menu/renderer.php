...
/* END CORE MOD */
    ...
    public function render_link($link, $course, $hr = false)
    {
        ...
        if ($link['icon']) {
/* BEGIN CORE MOD */
            $icon = $this->icon($link['icon'], $link['name'], array('class' => 'smallicon navicon customicon'));
/* END CORE MOD */
        }
        $attr = array();
        if ($link['target']) { // open in new window
/* BEGIN CORE MOD */
            $windowmoodleurl = new moodle_url($url);
            // Add inpopup param if internal
            if (stristr($windowmoodleurl->out_omit_querystring(), preg_replace('/^http(s)?:\/\//i', '', $CFG->wwwroot))) {
                $windowmoodleurl->param('inpopup', 1);
            }
            $windowurl = $windowmoodleurl->out(false);
            $windowname = str_ireplace(' ', '_', $link['name']);
            $attr['onclick'] = <<<HTML
window.open('{$windowurl}', '{$windowname}', 'resizable={$link['allowresize']},scrollbars={$link['allowscroll']},' +
