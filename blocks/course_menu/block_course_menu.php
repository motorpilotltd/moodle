...
/* BEGIN CORE MOD */
        'calendar', 'sitepages', 'profile', 'mycourses', 'myprofilesettings'
/* END CORE MOD */
    );
...
/* BEGIN CORE MOD */
                                $_course_node = $node_collection->find($this->page->course->id, global_navigation::TYPE_COURSE);
                                if (!$_course_node) {
                                    break;
                                }
                                $_node = $_course_node->children->get('participants');
/* END CORE MOD */
                                //check capabilities
...
/* BEGIN CORE MOD */
                                if (!empty($currentcontext) && get_config('local_delegatelist', 'version')) {
                                    try {
                                        require_once $CFG->dirroot.'/local/delegatelist/lib.php';
                                        $delegatelist = new delegate_list($this->course, $currentcontext);
                                        // Delegate list link.
                                        $element['url'] = $CFG->wwwroot . '/local/delegatelist/index.php?contextid=' . $currentcontext->id;
                                        $child_node = new navigation_node(array(
                                            'text' => get_string('coursemenulink', 'local_delegatelist'),
                                            'shorttext' => get_string('coursemenulink', 'local_delegatelist'),
                                            'icon' => new pix_icon('i/users', get_string('coursemenulink', 'local_delegatelist')),
                                            'action' => $element['url']
                                        ));
                                        $_node->add_node($child_node);
                                    } catch (Exception $e) {
...
/* END CORE MOD */
    ...
    function init_default_config($save_it = true)
    {
        ...
        $elements [] = $this->create_element(
/* BEGIN CORE MOD */
                'profile', get_string("profile"), '', '', 1, $this->_site_level, 1
/* END CORE MOD */
        );
...
/* BEGIN CORE MOD */
                if (in_array($elementId, array('sitepages', 'mycourses', 'profile'))) {
/* END CORE MOD */
    ...
    function config_elements()
    {
        ...
        if ($this->page->course->id == SITEID) {
            $elements = array();
/* BEGIN CORE MOD */
            $allowed = array('calendar', 'sitepages', 'profile', 'mycourses', 'myprofilesettings');
/* END CORE MOD */
            foreach ($this->config->elements as $element) {
