...
class apform_impactplan extends moodleform {
    public function definition() {
        ...
        $mform->addElement('hidden', 'page', 'impactplan');
        ...
        $mform->disabledIf('impact', 'appraiseeedit', 'eq', APPRAISAL_FIELD_LOCKED);

        $mform->addElement('textarearup', 'support', $this->str('support'), 'rows="3" cols="70"' . $appraiseelocked, $this->str('supporthelp'), 'appraisee');
        $mform->setType('support', PARAM_RAW);
        $mform->disabledIf('support', 'appraiseeedit', 'eq', APPRAISAL_FIELD_LOCKED);
    ...
    private function str($string) {
        return get_string('form:impactplan:' . $string, 'local_onlineappraisal');
    }
