 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class MoodleQuickForm_arupadvcheckbox extends HTML_QuickForm_advcheckbox implements templatable {
    ...
    function toHtml()
    {
        ...
        if ($this->_flagFrozen) {
            $input = $this->getFrozenHtml();
            $hiddeninput = '';
        } else {
            $input = '<input' . $this->_getAttrString($this->_attributes) . ' />';
            $hiddeninput = '<input' . $this->_getAttrString(array(
                    'type'  => 'hidden',
                    'name'  => $this->getName(),
