...
define(['jquery', 'core/config', 'core/str', 'core/templates', 'core/notification', 'local_onlineappraisal/autosize'],
    ...
    return /** @alias module:local_onlineappraisal/comment */ {
         ...
         * @method init
         */
        init: function(commenttemplate) {
            // Show more functionality.
            var comments = $('.oa-comments > ul > li');
            if (comments.length > 4) {
                comments.filter(':gt(3)').hide();
                var button = $('button.oa-comments-more');
                button.parent().removeClass('hidden');
                button.click(function(){
                    comments.filter(':hidden').filter(':lt(4)').show();
                    $(this).blur();
                    if (comments.filter(':hidden').length === 0) {
                        $(this).hide();
                    }
                });
            }

            // Adding comment to appraisal.
            autosize($('#oa-comment-input'));
            $('#oa-comment-submit').click(function(e){
                e.preventDefault();
                var self = $(this);
                var alert = $('.oa-comments-alert');
                var form = self.closest('form');
                var comment = form.find('#oa-comment-input');
                    ...
                    $.ajax({
                        ...
                        success: function(data){
                            ...
                            if (data.success) {
                                ...
                                templates.render(commenttemplate, data.data).done(function(html) {
                                    $('.oa-comments ul').prepend(html);
                                }).fail(notification.exception);
                            } else {
