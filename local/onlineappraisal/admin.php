...
$PAGE->set_url('/local/onlineappraisal/admin.php');
...
$page = optional_param('page', 'allstaff', PARAM_ALPHA);

try {
    \local_onlineappraisal\user::loginas_check();
    $admin = new \local_onlineappraisal\admin($page, $groupid, $cohortid);
    $admin->setup_page();
    $admin->prepare_page();
} catch (Exception $e) {
...
$url = $PAGE->url;
$url->params(array('groupid' => $admin->groupid, 'cohortid' => $admin->cohortid, 'page' => $admin->page));
$PAGE->set_url($url);
$PAGE->set_title($admin->pagetitle);
$PAGE->set_heading($admin->pageheading);
...
$PAGE->requires->js_call_amd('local_onlineappraisal/admin', 'init', array($arguments));
...
$PAGE->navbar->add(get_string($admin->page, 'local_onlineappraisal'), $PAGE->url);

echo $OUTPUT->header();

echo $admin->main_content();

echo $OUTPUT->footer();

$event = \local_onlineappraisal\event\appraisal_admin_viewed::create(array(
    'other' => array(
        'page' => $admin->page,
        'group' => $admin->groupid,
        'cohort' => $admin->cohortid,
    ),
));
$event->trigger();