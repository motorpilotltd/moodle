...
$cohort = \local_dynamic_cohorts\dynamic_cohorts::add_dynamic_cohort_fields($cohort);
...
if ($delete and $cohort->id) {
    ...
    if ($confirm and confirm_sesskey()) {
        ...
        \local_dynamic_cohorts\dynamic_cohorts::delete_dynamic_cohort($cohort->id, true);
        redirect($returnurl);
    }
...
$systemcontextroles = get_assignable_roles(context_system::instance(), ROLENAME_BOTH);
$editform = new cohort_edit_form(
    null,
    array(
...
if ($editform->is_cancelled()) {
    ...
    if(!empty($post['rulesetid'])){
        $data->rulesets = $post['rulesetid'];
        $data->ruleoperator = $post['ruleoperator'];
