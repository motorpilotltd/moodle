...
/**
 * Iconmulti coursemetadata field.
 *
...
/**
 * Class coursemetadata_field_iconmulti.
 *
 * @copyright  2016 Motorpilot Ltd
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class coursemetadata_field_iconmulti extends \local_coursemetadata\field_base {
    /** @var array $options */
    public $options;
    ...
    public function display_data() {
        ...
        foreach ($selections as $selection) {
            ...
            foreach ($types as $type) {
                if ($fs->file_exists($contextid, 'local_coursemetadata', $filearea, 0, '/', $selection.$type)) {
                    $imagesrc = "{$CFG->wwwroot}/pluginfile.php/{$contextid}/local_coursemetadata/{$filearea}/{$selection}{$type}";
                    break;
                }
            }
            if ($imagesrc) {
                $return .= html_writer::empty_tag(
                    ...
                    array(
                        ...
                        'alt' => $selection,
                        'title' => $selection,
                        'class' => 'coursemetadata iconmulti'
                    )
                );
            } else {
                $return .= html_writer::tag('span', $selection, array('class' => 'coursemetadata iconmulti'));
            }
        }
        return $return;
    }
    ...
    /**
     * Extra deletion tasks required when a field is removed.
     */
