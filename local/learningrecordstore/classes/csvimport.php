...
class csvimport {
    ...
    public function execute($tracker = null) {
        ...
        if (empty($tracker)) {
            $tracker = new \local_learningrecordstore\csvimporttracker();
        }
        $tracker->start();
    ...
    private function mkwarn($string) {
        $string = get_string('cpd:error:' . $string, 'local_learningrecordstore');
        return html_writer::tag('span', $string, array('class' => 'label label-warning'));
    }
    private function mkerr($string, $vars) {
        $string = get_string('cpd:error:' . $string, 'local_learningrecordstore', $vars);
    ...
    protected function validate() {
        foreach ($this->requiredcolumns as $requiredcolumn) {
            if (!in_array($requiredcolumn, $this->columns)) {
                throw new moodle_exception('csvloaderror', 'local_learningrecordstore');
            }
        }
