...
class csvimporttracker {
    ...
    public function finish() {
        ...
        echo $OUTPUT->render_from_template('local_learningrecordstore/csvupload_finish', $template);
    }
    ...
    public function output($line, $outcome, $status, $data) {
        ...
        echo $OUTPUT->render_from_template('local_learningrecordstore/csvupload', $template);
    }
    ...
    public function start() {
        ...
        $head = new stdClass();
        $head->name = get_string('form:csv:csvline', 'local_learningrecordstore');
        $head->class = 'c' . $ci++;
        $template->heading[] = $head;


        $head = new stdClass();
        $head->name = get_string('form:csv:result', 'local_learningrecordstore');
        $head->class = 'c' . $ci++;
        $template->heading[] = $head;

        foreach ($this->columns as $col) {
            $head = new stdClass();
            $head->name = get_string('form:csv:'. $col, 'local_learningrecordstore');
            $head->class = 'c' . $ci++;
            $template->heading[] = $head;
        }

        echo $OUTPUT->render_from_template('local_learningrecordstore/csvupload_start', $template);
    }
}
