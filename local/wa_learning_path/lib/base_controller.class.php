...
//        $PAGE->set_url(new \moodle_url($this->url, array('c' => $this->c, 'a' => $this->a)));
     ...
     * @return string
     */
    public function get_string($text, $a = null) {
        return get_string($text, 'local_wa_learning_path', $a);
    }

    public function string_exists($text) {
        return \get_string_manager()->string_exists($text, 'local_wa_learning_path');
    }
    /**
     * Render view: Header + Content + Footer. If request is AJAX render only Content, without Header and Footer.
