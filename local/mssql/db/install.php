...
function xmldb_local_mssql_install() {
    ...
    if ($DB->get_dbfamily() == 'mssql' && $serverinfo['description'] !== 'mssqlubuntu') {
        ...
        if (!$DB->get_record_sql('SELECT * FROM sys.assemblies WHERE name = ?',
            ...
            $sqllist = require($CFG->dirroot . '/local/mssql/db/mssqlgroupconcat.php');
            try {
                ...
                echo $OUTPUT->notification(get_string('mssqlgroupconcatfail', 'local_mssql',
                        $message));
 ...
 */
function xmldb_local_mssql_install_recovery() {
    xmldb_local_mssql_install();
}
 ...
 */
function local_mssql_environment_check(environment_results $result) {
    global $DB;
    $status = true;
 ...
 * @return environment_results
 */
function local_mssql_environment_read_committed_snapshot(environment_results $result) {
    global $DB, $CFG;
    ...
    if (!$on) {
        $result->setRestrictStr(array('requiredreadcommittedsnapshot', 'local_mssql'));
