...
function arup_local_kaltura_get_kaltura_client($privileges = '', $expiry = '') {
    ...
    try {
        $ks = $client->generateSessionV2($configsettings->adminsecret, $USER->username, KalturaSessionType::ADMIN, $configsettings->partner_id, $expiry, $privileges);
        $client->setKs($ks);
    } catch (Exception $ex) {
        $client = null;
        error_log('Could not connect to Kaltura.');
    }
