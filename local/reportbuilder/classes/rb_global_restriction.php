...
class rb_global_restriction {
    ...
    protected function load($id) {
        ...
        $restriction = $DB->get_record('report_builder_global_restr', array('id' => $id), '*', MUST_EXIST);
    ...
    public function insert($data) {
        ...
        $max = $DB->get_field('report_builder_global_restr', 'MAX(sortorder)', array());
        $data->sortorder = ($max === null ? 0 : intval($max) + 1);

        $id = $DB->insert_record('report_builder_global_restr', $data);
    ...
    public function update($data) {
        ...
        $DB->update_record('report_builder_global_restr', $data);
    ...
    public function delete() {
        ...
        $DB->delete_records('reportbuilder_grp_cht_rec', array ('reportbuilderrecordid' => $this->id));
        $DB->delete_records('reportbuilder_grp_usr_rec', array ('reportbuilderrecordid' => $this->id));

        $DB->delete_records('reportbuilder_grp_cht_usr', array ('reportbuilderuserid' => $this->id));
        $DB->delete_records('reportbuilder_grp_usr_usr', array ('reportbuilderuserid' => $this->id));

        $DB->delete_records('report_builder_global_restr', array ('id' => $this->id));
    }
    ...
    public function activate() {
        ...
        $this->active = '1';
        $DB->set_field('report_builder_global_restr', 'active', $this->active, array('id' => $this->id));
    }
    ...
    protected function move($up) {
        ...
        if ($up) {
            $sql = "SELECT id, sortorder
                      FROM {report_builder_global_restr}
                     WHERE sortorder < ?
        ...
        if ($switch) {
            $switch = reset($switch);
            $DB->set_field('report_builder_global_restr', 'sortorder', $this->sortorder, array('id' => $switch->id));
            $DB->set_field('report_builder_global_restr', 'sortorder', $switch->sortorder, array('id' => $this->id));
        }

        // Reorder just in case.
        $sort = 0;
        $records = $DB->get_records('report_builder_global_restr', array(), 'sortorder ASC', 'id, sortorder');
        foreach ($records as $record) {
            if ($record->sortorder != $sort) {
                $DB->set_field('report_builder_global_restr', 'sortorder', $sort, array('id' => $record->id));
            }
            $sort++;
        }
    ...
    public static function get_all($offset = 0, $limit = 0, &$count) {
        global $DB, $CFG;
        require_once($CFG->dirroot . '/local/reportbuilder/lib/assign/lib.php');

        $restrictions = $DB->get_records('report_builder_global_restr', array(), 'sortorder ASC', '*', $offset*$limit, $limit);
        $count = $DB->count_records('report_builder_global_restr');
