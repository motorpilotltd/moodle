...
class rb_column {
     ...
     *
     * 'grouporder' => array('certif_coursesets.sortorder', 'certif_coursesets_course.id')
     */
    public $grouporder;
    ...
    public function get_fields($src = null, $aliasmode = self::REGULAR, $returnextrafields=true) {
        ...
        if ($field === null) {
            ...
            $classname = "\\local_reportbuilder\\rb\\transform\\$this->transform";
            if (class_exists($classname)) {
                $fields[] = $classname::get_field($this, $src, $aliasmode, $returnextrafields);
            }

        } else if ($this->aggregate) {
            $classname = "\\local_reportbuilder\\rb\\aggregate\\$this->aggregate";
            if (class_exists($classname)) {
                $fields[] = $classname::get_field($this, $src, $aliasmode, $returnextrafields);
            }
    ...
    /**
     * Returns class name of local_reportbuilder\rb\display\ class used for
    ...
    public function get_display_class(reportbuilder $report) {
        ...
        if (!$displayfunc) {
            return '\local_reportbuilder\rb\display\legacy';
        }
