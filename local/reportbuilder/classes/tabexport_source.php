 ...
 * @package totara_core
 */
class tabexport_source implements \Iterator {
    ...
    public function get_extra_information() {
        ...
        if (is_array($restrictions) && count($restrictions) > 0) {
            $result[] = get_string('reportcontents', 'local_reportbuilder');
        ...
        if ($this->cache) {
            ...
            $result[] = get_string('report:cachelast', 'local_reportbuilder', $a);
        }
    ...
    public function get_svg_graph($w, $h) {
        ...
        $graph = new \local_reportbuilder\local\graph($this->report);
        if (!$graph->is_valid()) {
            return null;
        }
