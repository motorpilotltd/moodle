...
class role extends base {
    ...
    public function form_template($mform, $reportid) {
        ...
        if (!empty($roles)) {
            ...
            $mform->addHelpButton('role_context', 'reportbuildercontext', 'local_reportbuilder');
            ...
            $mform->addGroup($rolesgroup, 'roles', get_string('roleswithaccess', 'local_reportbuilder'), html_writer::empty_tag('br'), false);
            $mform->disabledIf('roles', 'accessenabled', 'eq', 0);
            $mform->addHelpButton('roles', 'reportbuilderrolesaccess', 'local_reportbuilder');
        } else {
            $mform->addElement('html', html_writer::tag('p', get_string('error:norolesfound', 'local_reportbuilder')));
        }
    }
