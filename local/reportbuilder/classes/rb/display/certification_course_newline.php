 ...
 */
class certification_course_newline extends certification_course_base {
    ...
    public static function display($value, $format, \stdClass $row, \rb_column $column, \reportbuilder $report) {
        ...
        foreach ($items as $key => $item) {
            list($certificationid, $courseid, $data) = explode('|', $item);
            $data = trim($data);
            if (empty($data) || $data === '-') {
                $newitems[$key] = '-';
            } else {
        ...
        if ($certificationid && count($newitems) > 1 && self::resort_required()) {
            $newitems = self::resort($certificationid, $newitems, $reference);
        }
