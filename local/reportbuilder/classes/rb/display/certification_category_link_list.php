 ...
 * @package totara_certification
 */
class certification_category_link_list extends certification_course_base {
    ...
    public static function display($value, $format, \stdClass $row, \rb_column $column, \reportbuilder $report) {
        ...
        foreach ($items as $key => $item) {
            list($certificationid, $courseid, $catid, $visible, $catname) = explode('|', $item);
            if ($visible && !$isexport) {
        ...
        if ($certificationid && count($reference) > 1 && self::resort_required()) {
