...
$capability = $rawreport->embedded ? 'local/reportbuilder:manageembeddedreports' : 'local/reportbuilder:managereports';
...
switch ($action) {
    case 'add' :
        ...
        if ($customheading) {
            if (trim($heading) === '') {
                $result->error = get_string('noemptycols', 'local_reportbuilder');
        ...
        if ($DB->record_exists('report_builder_columns', $params)) {
            $result->error = get_string('norepeatcols', 'local_reportbuilder');
            break;
        }
    ...
    case 'delete':
        ...
        $graphseries = $DB->get_field_sql("select series from {report_builder_graph} where reportid = :reportid and type <> ''",['reportid' => $reportid]);
        ...
        if ($column) {
            ...
            if (get_string_manager()->string_exists('type_' . $column->type, 'rbsource_' . $column->source)) {
                $column->optgroup_label = get_string('type_' . $column->type, 'rbsource_' . $column->source);
                // How about in report builder?
            } else if (get_string_manager()->string_exists('type_' . $column->type, 'local_reportbuilder')) {
                $column->optgroup_label = get_string('type_' . $column->type, 'local_reportbuilder');
            } else {
                // Not found, display in missing string format to make it obvious.
                $column->optgroup_label = get_string_manager()->get_string('type_' . $column->type, 'rbsource_' . $column->source);
            }
