    ...
    foreach ($keywords as $keyword) {
        ...
        foreach ($fields as $field) {
            if ($type == SQL_PARAMS_QM) {
                ...
                $paramname = \local_reportbuilder\dblib\base::getbdlib()->get_unique_param($prefix);
...
function search_get_keyword_where_clause_options($field, $keywords, $negate=false, $operator='contains', $fieldisvalue=false) {
    ...
    foreach ($keywords as $keyword) {
        $uniqueparam = \local_reportbuilder\dblib\base::getbdlib()->get_unique_param($operator);
        if (!$fieldisvalue) {
            $queries[] = $DB->sql_like($field, ":{$uniqueparam}", false, true, $not);
        } else {
            // Sometimes we want to pass value in instead of used db field.
            $fieldparam = \local_reportbuilder\dblib\base::getbdlib()->get_unique_param('fieldval');
