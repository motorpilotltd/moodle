...
class email_setting_schedule {
    ...
    public static function get_system_users_to_email($scheduleid) {
        ...
        $sql = 'SELECT ses.userid AS id, ' . $usernamefields . '
                  FROM {report_builder_schd_eml_user} ses
             LEFT JOIN {user} u
                    ON ses.userid = u.id
    ...
    public static function get_external_users_to_email($scheduleid) {
        ...
        return $DB->get_fieldset_select('report_builder_schd_eml_ext', 'email', $select, $params);
    }
    ...
    protected function get_emails_audiences() {
        ...
        $sql = "SELECT DISTINCT u.*
              FROM {report_builder_schd_eml_aud} sea
    ...
    private function add_email_setting($table, $field, $emails) {
        ...
        if (!empty($emails)) {
            ...
            return $DB->insert_records($table, $todb);
        }
    }
     ...
     */
    public function set_email_settings(array $systemusers, array $externalusers) {
        global $DB;

        // Get current values settings to determine if we need to delete or create some records.
        $select = 'scheduleid = :scheduleid';
        $param = array('scheduleid' => $this->schedule->id);
        $currentsystemusers = $DB->get_fieldset_select('report_builder_schd_eml_user', 'userid', $select, $param);
        $currentexternalusers = $DB->get_fieldset_select('report_builder_schd_eml_ext', 'email', $select, $param);
        ...
        // Deleting records.
        $this->delete_email_setting('report_builder_schd_eml_user', 'userid', $sysuserstodelete);
        $this->delete_email_setting('report_builder_schd_eml_ext', 'email', $extuserstodelete);

        // Inserting records.
        $this->add_email_setting('report_builder_schd_eml_user', 'userid', $sysuserstoinsert);
        $this->add_email_setting('report_builder_schd_eml_ext', 'email', $extuserstoinsert);
    }
}
