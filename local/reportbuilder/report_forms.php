...
class report_builder_new_form extends moodleform {
    ...
    function definition() {
        ...
        if (count($sources) > 0) {
            ...
            $mform->addHelpButton('fullname', 'reportbuilderfullname', 'local_reportbuilder');
            ...
            $mform->addHelpButton('source', 'reportbuildersource', 'local_reportbuilder');

            $mform->addElement('advcheckbox', 'hidden', get_string('hidden', 'local_reportbuilder'), '', null, array(0, 1));
            $mform->addHelpButton('hidden', 'reportbuilderhidden', 'local_reportbuilder');
...
class report_builder_edit_form extends moodleform {
    function definition() {
        ...
        $mform->addHelpButton('description_editor', 'reportbuilderdescription', 'local_reportbuilder');

        $mform->addElement('static', 'reportsource', get_string('source', 'local_reportbuilder'), $report->src->sourcetitle);
        $mform->addHelpButton('reportsource', 'reportbuildersource', 'local_reportbuilder');
        ...
        $mform->addElement('text', 'recordsperpage', get_string('recordsperpage', 'local_reportbuilder'), array('size' => '6', 'maxlength' => 4));
        $mform->setType('recordsperpage', PARAM_INT);
        $mform->addRule('recordsperpage', null, 'numeric');
        $mform->addHelpButton('recordsperpage', 'reportbuilderrecordsperpage', 'local_reportbuilder');
...
class report_builder_edit_filters_form extends moodleform {
    function definition() {
        ...
        $mform->addElement('header', 'standardfilter', get_string('standardfilter', 'local_reportbuilder'));
        $mform->addHelpButton('standardfilter', 'standardfilter', 'local_reportbuilder');
        ...
        if (isset($report->filteroptions) && is_array($report->filteroptions) && count($report->filteroptions) > 0) {
            ...
            $mform->addElement('html', $OUTPUT->container(get_string('standardfilterdesc', 'local_reportbuilder')) .
            ...
            $mform->addElement('html', $OUTPUT->container_start('reportbuilderform') . html_writer::start_tag('table', array('class' => 'table table-striped')) .
                    html_writer::start_tag('tr') . html_writer::tag('th', get_string('searchfield', 'local_reportbuilder')) .
                    html_writer::tag('th', get_string('customisename', 'local_reportbuilder'), array('colspan' => 2)) .
                    html_writer::tag('th', get_string('advanced', 'local_reportbuilder')) .
                    html_writer::tag('th', get_string('options', 'local_reportbuilder')) . html_writer::end_tag('tr'));
            ...
            if (isset($report->filters) && is_array($report->filters) && count($report->filters) > 0) {
                ...
                foreach ($filters as $index => $filter) {
                    ...
                    $mform->addElement('html', html_writer::end_tag('td') . html_writer::start_tag('td'));
                    $deleteurl = new moodle_url('/local/reportbuilder/filters.php',
                    ...
                    if ($i != 1) {
                        $moveupurl = new moodle_url('/local/reportbuilder/filters.php',
                    ...
                    if ($i != $standardfiltercount) {
                        $movedownurl = new moodle_url('/local/reportbuilder/filters.php',
                ...
                $mform->addElement('html', html_writer::tag('p', get_string('nofiltersyet', 'local_reportbuilder')));
            }
            ...
            $mform->addElement('header', 'sidebarfilter', get_string('sidebarfilter', 'local_reportbuilder'));
            $mform->addHelpButton('sidebarfilter', 'sidebarfilter', 'local_reportbuilder');
            $mform->setExpanded('sidebarfilter');

            $mform->addElement('html', $OUTPUT->container(get_string('sidebarfilterdesc', 'local_reportbuilder')) .
                    html_writer::empty_tag('br'));
            ...
            $mform->addElement('html', get_string('nofilteraskdeveloper', 'local_reportbuilder', $report->source));
        }

        // Toolbar search options.
        $mform->addElement('header', 'toolbarsearch', get_string('toolbarsearch', 'local_reportbuilder'));
        $mform->addHelpButton('toolbarsearch', 'toolbarsearch', 'local_reportbuilder');
        $mform->setExpanded('toolbarsearch');

        $mform->addElement('advcheckbox', 'toolbarsearchdisabled', get_string('toolbarsearchdisabled', 'local_reportbuilder'));
        $mform->setDefault('toolbarsearchdisabled', !$report->toolbarsearch);
        $mform->addHelpButton('toolbarsearchdisabled', 'toolbarsearchdisabled', 'local_reportbuilder');

        if (count($allsearchcolumns) > 0) {
            $searchcolumns = $report->searchcolumns;

            $mform->addElement('html', $OUTPUT->container(get_string('toolbarsearchdesc', 'local_reportbuilder')) .
                    html_writer::empty_tag('br'));
            ...
            if (isset($report->searchcolumns) && is_array($report->searchcolumns) && count($report->searchcolumns) > 0) {
                ...
                $mform->addElement('html', html_writer::tag('p', get_string('nosearchcolumnsyet', 'local_reportbuilder')));
            }
            ...
            $mform->addElement('html', get_string('nosearchcolumnsaskdeveloper', 'local_reportbuilder', $report->source));
        }
...
class report_builder_edit_columns_form extends moodleform {
    ...
    function definition() {
        ...
        $mform->addHelpButton('reportcolumns', 'reportbuildercolumns', 'local_reportbuilder');
        ...
        if (isset($report->columnoptions) && is_array($report->columnoptions) && count($report->columnoptions) > 0) {
            ...
            if ($report->grouped) {
                if ($report->is_internally_grouped()) {
                    $mform->addElement('html', $OUTPUT->notification(get_string('warngroupaggregation', 'local_reportbuilder'), 'notifymessage'));
                }
            }
            if ($report->requiredcolumns) {
                $mform->addElement('html', $OUTPUT->notification(get_string('warnrequiredcolumns', 'local_reportbuilder'), 'notifymessage'));
            }

            $mform->addElement('html', $OUTPUT->container(get_string('help:columnsdesc', 'local_reportbuilder')) .
                html_writer::empty_tag('br'));


            $mform->addElement('html', $OUTPUT->container_start('reportbuilderform') . html_writer::start_tag('table', array('class' => 'table table-striped')) .
                html_writer::start_tag('tr') . html_writer::tag('th', get_string('column', 'local_reportbuilder')) .
                html_writer::tag('th', get_string('advancedcolumnheading', 'local_reportbuilder')) .
                html_writer::tag('th', get_string('customiseheading', 'local_reportbuilder'), array('colspan' => 2)) .
                html_writer::tag('th', get_string('options', 'local_reportbuilder') . html_writer::end_tag('tr')));

            $columnsselect = $report->get_columns_select();
            $defaultoptions = array('' => get_string('noneselected', 'local_reportbuilder'));
            ...
            if ($goodcolumns) {
                ...
                foreach ($goodcolumns as $cid => $column) {
                    ...
                    if (!isset($column->required) || !$column->required) {
                        ...
                        // show/hide link
                        if ($column->hidden == 0) {
                            $hideurl = new moodle_url('/local/reportbuilder/columns.php',
                            ...
                            $showurl = new moodle_url('/local/reportbuilder/columns.php',
                        ...
                        // delete link
                        $delurl = new moodle_url('/local/reportbuilder/columns.php',
                        ...
                        // move up link
                        if ($i != 1) {
                            $moveupurl = new moodle_url('/local/reportbuilder/columns.php',
                        ...
                        // move down link
                        if ($i != $colcount) {
                            $movedownurl = new moodle_url('/local/reportbuilder/columns.php',
                ...
                $mform->addElement('html', html_writer::tag('p', get_string('nocolumnsyet', 'local_reportbuilder')));
            }
            ...
            if ($badcolumns) {
                ...
                $mform->addElement('html', html_writer::tag('p', get_string('badcolumnsdesc', 'local_reportbuilder')));
                ...
                $mform->addElement('html',
                    ...
                    html_writer::tag('th', get_string('type', 'local_reportbuilder')) .
                    html_writer::tag('th', get_string('value', 'local_reportbuilder')) .
                    html_writer::tag('th', get_string('heading', 'local_reportbuilder')) .
                    html_writer::tag('th', get_string('options', 'local_reportbuilder')) . html_writer::end_tag('tr'));
                foreach ($badcolumns as $bad) {
                    $deleteurl = new moodle_url('/local/reportbuilder/columns.php',
            ...
            $mform->addHelpButton('sorting', 'reportbuildersorting', 'local_reportbuilder');

            $mform->addElement('select', 'defaultsortcolumn', get_string('defaultsortcolumn', 'local_reportbuilder'), $defaultoptions);
            ...
            $mform->addGroup($radiogroup, 'radiogroup', get_string('defaultsortorder', 'local_reportbuilder'), html_writer::empty_tag('br'), false);
        ...
        foreach ($data as $key => $value) {
            ...
            if (preg_match('/^column\d+/', $key) or ($key === 'newcolumns' and $value)) {
                if (isset($usedcols[$value])) {
                    $errors[$key] = get_string('norepeatcols', 'local_reportbuilder');
                } else {
                    $usedcols[$value] = true;
                }
                continue;
...
class report_builder_edit_graph_form extends moodleform {
    public function definition() {
        ...
        $optionoptions = array(
            'C' => get_string('graphorientationcolumn', 'local_reportbuilder'),
            'R' => get_string('graphorientationrow', 'local_reportbuilder'),
        );
        $mform->addElement('select', 'orientation', get_string('graphorientation', 'local_reportbuilder'), $optionoptions);
        $mform->addHelpButton('orientation', 'graphorientation', 'local_reportbuilder');

        $mform->addElement('header', 'serieshdr', 'Data');

        $catoptions = array('none' => get_string('graphnocategory', 'local_reportbuilder'));
        $legendoptions = array();
        $series = array();
        ...
        $mform->addElement('select', 'category', get_string('graphcategory', 'local_reportbuilder'), $catoptions);
        $mform->disabledIf('category', 'type', 'eq', '');
        $mform->disabledIf('category', 'orientation', 'noteq', 'C');

        $mform->addElement('select', 'legend', get_string('graphlegend', 'local_reportbuilder'), $legendoptions);
        $mform->disabledIf('legend', 'type', 'eq', '');
        $mform->disabledIf('legend', 'orientation', 'noteq', 'R');

        if ($series) {
            $mform->addElement('select', 'series', get_string('graphseries', 'local_reportbuilder'), $series, array('multiple' => true));
            $mform->disabledIf('series', 'type', 'eq', '');
        } else {
            unset($graph->series);
            $mform->addElement('static', 'series', get_string('graphseries', 'local_reportbuilder'), get_string('error:nographseries', 'local_reportbuilder'));
        }
        ...
        $mform->addElement('text', 'maxrecords', get_string('graphmaxrecords', 'local_reportbuilder'));
        $mform->setType('maxrecords', PARAM_INT);
        $mform->disabledIf('maxrecords', 'type', 'eq', '');

        $mform->addElement('textarea', 'settings', get_string('graphsettings', 'local_reportbuilder'), array('rows' => 10));
        $mform->addHelpButton('settings', 'graphsettings', 'local_reportbuilder');
    ...
    function definition() {
        ...
        if (count($contentoptions)) {
            ...
            if (count($contentoptions)) {
                ...
                $mform->addGroup($radiogroup, 'radiogroup', get_string('restrictcontent', 'local_reportbuilder'), html_writer::empty_tag('br'), false);
                $mform->addHelpButton('radiogroup', 'reportbuildercontentmode', 'local_reportbuilder');
...
class report_builder_edit_access_form extends moodleform {
    function definition() {
        ...
        if ($report->embeddedurl !== null) {
            $mform->addElement('html', html_writer::tag('p', get_string('embeddedaccessnotes', 'local_reportbuilder')));
        }
        $radiogroup = array();
...
class report_builder_edit_performance_form extends moodleform {
    function definition() {
        ...
        if (get_config('local_reportbuilder', 'allowtotalcount')) {
            ...
            $mform->addElement('header', 'generalperformance', get_string('generalperformancesettings', 'local_reportbuilder'));
            $mform->setExpanded('generalperformance');
            $mform->addElement('checkbox', 'showtotalcount', get_string('showtotalcount', 'local_reportbuilder'));
            $mform->addHelpButton('showtotalcount', 'showtotalcount', 'local_reportbuilder');
        }

        $mform->addElement('header', 'filterperformance', get_string('initialdisplay_heading', 'local_reportbuilder'));
        ...
        if (get_config('local_reportbuilder', 'globalinitialdisplay') && !$report->embedded) {
            ...
            $initial_display_sidenote = get_string('globalinitialdisplay_enabled', 'local_reportbuilder');
            ...
            $initial_display_sidenote = is_null($initial_display_attributes) ? '' : get_string('initialdisplay_disabled', 'local_reportbuilder');
        }
        $mform->addElement('advcheckbox', 'initialdisplay', get_string('initialdisplay', 'local_reportbuilder'),
        ...
        $mform->addHelpButton('initialdisplay', 'initialdisplay', 'local_reportbuilder');
        $mform->addElement('header', 'cachingperformance', get_string('reportbuildercache_heading', 'local_reportbuilder'));
        ...
        if (!$problems) {
            ...
            $caching_sidenote = is_null($caching_attributes) ? '' :
                    get_string('reportbuildercache_disabled', 'local_reportbuilder');
            $mform->addElement('advcheckbox', 'cache', get_string('cache', 'local_reportbuilder'),
                    $caching_sidenote, $caching_attributes, array(0, 1));
            $mform->setType('cache', PARAM_INT);
            $mform->addHelpButton('cache', 'reportbuildercache', 'local_reportbuilder');

            $mform->addElement('scheduler', 'schedulegroup', get_string('reportbuildercachescheduler', 'local_reportbuilder'));
            $mform->disabledIf('schedulegroup', 'cache');
            $mform->addHelpButton('schedulegroup', 'reportbuildercachescheduler', 'local_reportbuilder');
            ...
            $mform->addElement('static', 'servertime', get_string('reportbuildercacheservertime', 'local_reportbuilder'), $servertime);
            $mform->addHelpButton('servertime', 'reportbuildercacheservertime', 'local_reportbuilder');

            $cachetime = isset($report->cacheschedule->lastreport) ? $report->cacheschedule->lastreport : 0;
            $cachedstr = get_string('lastcached','local_reportbuilder', userdate($cachetime));
            $notcachedstr = get_string('notcached','local_reportbuilder');
            $lastcached = ($cachetime > 0) ? $cachedstr : $notcachedstr;

            if ($report->cache) {
                $mform->addElement('static', 'cachenowselector', get_string('reportbuilderinitcache', 'local_reportbuilder'),
                    html_writer::tag('span', $lastcached. ' ') .
                    $output->cachenow_button($id)
                );
            } else {
                $mform->addElement('advcheckbox', 'generatenow', get_string('cachenow', 'local_reportbuilder'), '', null, array(0, 1));
                $mform->setType('generatenow', PARAM_INT);
                $mform->addHelpButton('generatenow', 'cachenow', 'local_reportbuilder');
        ...
        if (totara_is_clone_db_configured()) {
            $mform->addElement('header', 'useclonedbheader', get_string('useclonedbheader', 'local_reportbuilder'));
            $mform->setExpanded('useclonedbheader');
            $mform->addElement('checkbox', 'useclonedb', get_string('useclonedb', 'local_reportbuilder'));
            $mform->addHelpButton('useclonedb', 'useclonedb', 'local_reportbuilder');
...
class report_builder_save_form extends moodleform {
    function definition() {
        ...
        $mform->addRule('name', get_string('missingsearchname', 'local_reportbuilder'), 'required', null, 'server');
...
class report_builder_standard_search_form extends moodleform {
    function definition() {
        ...
        if ($fields && is_array($fields) && count($fields) > 0) {
            $mform->addElement('header', 'newfilterstandard', get_string('searchby', 'local_reportbuilder'));
...
class report_builder_sidebar_search_form extends moodleform {
    public function definition() {
        ...
        if ($fields && is_array($fields) && count($fields) > 0) {
            ...
            $mform->addElement('header', 'newfiltersidebar', get_string('filterby', 'local_reportbuilder'));
...
class report_builder_toolbar_search_form extends moodleform {
    public function definition() {
        ...
        $mform->addElement('text', 'toolbarsearchtext',
                get_string('searchby', 'local_reportbuilder'));
...
class report_builder_course_expand_form extends moodleform {
    public function definition() {
        ...
        if ($enroltype != '') {
            $mform->addElement('static', 'enroltype', get_string('courseenroltype', 'local_reportbuilder'), $enroltype);
        }
        if ($progress != '') {
            $mform->addElement('static', 'progress', get_string('courseprogress', 'local_reportbuilder'), $progress);
        }
        if ($enddate != '') {
            $mform->addElement('static', 'enddate', get_string('courseenddate', 'local_reportbuilder'), $enddate);
        }
        if ($grade != '') {
