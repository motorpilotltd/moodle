...
class local_reportbuilder_rb_global_restriction_set_testcase extends advanced_testcase {
    ...
    INNER JOIN (SELECT p.id, p.id AS instanceid,
            p.fullname AS name, p.icon,  CASE WHEN p.certifid > ? THEN ? ELSE ? END AS instancetype
 ...
 WHERE ef_419295f028_insid = ? AND ef_419295f028_type = ?";
    ...
    protected function setUp() {
        ...
        /** @var local_reportbuilder_generator $reportgen */
        $reportgen = $this->getDataGenerator()->get_plugin_generator('local_reportbuilder');
    ...
    public function test_create_from_selected_all() {
        ...
        $DB->delete_records('report_builder_global_restr', array('id' => $this->restrallusers->id));
        rb_global_restriction_set::get_user_all_restrictions($this->usernone->id, true);
        rb_global_restriction_set::get_user_all_restrictions($this->user->id, true);
