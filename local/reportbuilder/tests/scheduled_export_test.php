...
class local_reportbuilder_scheduled_export_testcase extends advanced_testcase {
    ...
    public function test_reportbuilder_send_scheduled_report() {
        ...
        $DB->insert_record('report_builder_schd_eml_user', array('scheduleid' => $schedule1->id, 'userid' => $user1->id));
        $DB->insert_record('report_builder_schd_eml_ext', array('scheduleid' => $schedule1->id, 'email' => 'xxx@example.com'));
        $DB->insert_record('report_builder_schd_eml_aud', array('scheduleid' => $schedule1->id, 'cohortid' => $audience->id));
        ...
        $DB->insert_record('report_builder_schd_eml_user', array('scheduleid' => $schedule2->id, 'userid' => $user1->id));
        $DB->insert_record('report_builder_schd_eml_ext', array('scheduleid' => $schedule2->id, 'email' => 'xxx@example.com'));
        $DB->insert_record('report_builder_schd_eml_aud', array('scheduleid' => $schedule2->id, 'cohortid' => $audience->id));
        ob_start(); // Verify diagnostic output.
        ...
        $DB->insert_record('report_builder_schd_eml_user', array('scheduleid' => $schedule3->id, 'userid' => $user1->id));
        $DB->insert_record('report_builder_schd_eml_ext', array('scheduleid' => $schedule3->id, 'email' => 'xxx@example.com'));
        $DB->insert_record('report_builder_schd_eml_aud', array('scheduleid' => $schedule3->id, 'cohortid' => $audience->id));
        ob_start(); // Verify diagnostic output.
    ...
    public function test_process_scheduled_task() {
        ...
        $schedule1->id = $DB->insert_record('report_builder_schedule', $schedule1);
        $schedule1 = $DB->get_record('report_builder_schedule', array('id' => $schedule1->id));
        $DB->insert_record('report_builder_schd_eml_user', array('scheduleid' => $schedule1->id, 'userid' => $admin->id));

        $sink = $this->redirectEmails();

        $task = new \local_reportbuilder\task\process_scheduled_task();
        ob_start(); // Verify diagnostic output.
        $task->execute();
        $info = ob_get_contents();
        ob_end_clean();
    ...
    public function test_process_scheduled_task_skip_inactive_users() {
        ...
        $schedule2->id = $DB->insert_record('report_builder_schedule', $schedule2);
        $schedule2 = $DB->get_record('report_builder_schedule', array('id' => $schedule2->id));
        $DB->insert_record('report_builder_schd_eml_user', array('scheduleid' => $schedule2->id, 'userid' => $user2->id));
        ...
        $schedule3->id = $DB->insert_record('report_builder_schedule', $schedule3);
        $schedule3 = $DB->get_record('report_builder_schedule', array('id' => $schedule3->id));
        $DB->insert_record('report_builder_schd_eml_user', array('scheduleid' => $schedule3->id, 'userid' => $user3->id));
    ...
    public function test_export_formats() {
        ...
        $schedules = array();
        $plugins = \local_reportbuilder\tabexport_writer::get_export_classes();
