...
$returnurl = new moodle_url('/local/reportbuilder/filters.php', array('id' => $id));
...
if ($d and $confirm) {
    if (!confirm_sesskey()) {
        notice(get_string('error:bad_sesskey', 'local_reportbuilder'), $returnurl);
    }
    if (isset($fid)) {
        if ($report->delete_filter($fid)) {
            \local_reportbuilder\event\report_updated::create_from_report($report, 'filters')->trigger();
            notice(get_string('filterdeleted', 'local_reportbuilder'), $returnurl,
                array('class' => 'notifysuccess'));
        } else {
            notice(get_string('error:filter_not_deleted', 'local_reportbuilder'), $returnurl);
        }
    } else if (isset($searchcolumnid)) {
        if ($report->delete_search_column($searchcolumnid)) {
            \local_reportbuilder\event\report_updated::create_from_report($report, 'filters')->trigger();
            notice(get_string('searchcolumndeleted', 'local_reportbuilder'), $returnurl,
                array('class' => 'notifysuccess'));
        } else {
            notice(get_string('error:search_column_not_deleted', 'local_reportbuilder'), $returnurl);
        }
    }
}
...
if ($d) {
    ...
    if (isset($fid)) {
        $confirmurl = new moodle_url('/local/reportbuilder/filters.php',
            array('d' => '1', 'id' => $id, 'fid' => $fid, 'confirm' => '1', 'sesskey' => $USER->sesskey));
        $confirmstr = get_string('confirmfilterdelete', 'local_reportbuilder');
        if ($initialdisplay && $sizeoffilters == 1) {
            $a = '';
            if ($globalinitialdisplay) {
                $a = get_string('confirmfilterdelete_grid_enabled', 'local_reportbuilder');
            }
            $confirmstr = get_string('confirmfilterdelete_rid_enabled', 'local_reportbuilder', $a);
        }
        ...
        $confirmstr = get_string('confirmsearchcolumndelete', 'local_reportbuilder');
        if ($initialdisplay && $sizeoffilters == 1) {
            $a = '';
            if ($globalinitialdisplay) {
                $a = get_string('confirmfilterdelete_grid_enabled', 'local_reportbuilder');
...
if ($m && isset($fid)) {
    if ($report->move_filter($fid, $m)) {
        \local_reportbuilder\event\report_updated::create_from_report($report, 'filters')->trigger();
        notice(get_string('filtermoved', 'local_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
    } else {
        notice(get_string('error:filter_not_moved', 'local_reportbuilder'), $returnurl);
    }
}
...
if ($mform->is_cancelled()) {
    redirect(new moodle_url('/local/reportbuilder/index.php'));
}
if ($fromform = $mform->get_data()) {
    ...
    if (build_filters($id, $fromform)) {
        ...
        \local_reportbuilder\event\report_updated::create_from_report($report, 'filters')->trigger();
        notice(get_string('filters_updated', 'local_reportbuilder'), $returnurl,
            array('class' => 'notifysuccess'));
    } else {
        notice(get_string('error:filters_not_updated', 'local_reportbuilder'), $returnurl);
    }
