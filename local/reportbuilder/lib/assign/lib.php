...
class totara_assign_core {
    ...
    /**
     * Id of the module instance - appraisalid, planid etc.
     *
     * @access  public
     * @var     int
     */
    ...
    public function load_grouptype($grouptype) {
        ...
        if (!file_exists($classfile)) {
            print_error('error:assignmentprefixnotfound', 'local_reportbuilder', '', $grouptype);
        }
    ...
    public function get_group_instance_name($grouptype, $groupid) {
        ...
        if (!in_array($grouptype, $groups)) {
            print_error('error:invalidgrouptype', 'local_reportbuilder');
        }
    ...
    public function delete_assigned_group($grouptype, $deleteid) {
        if (!in_array($grouptype, $this->get_assignable_grouptypes())) {
            print_error('error:assigncannotdeletegrouptypex', 'local_reportbuilder', $grouptype);
        }
        if ($this->is_locked()) {
            print_error('error:assignmentmoduleinstancelocked', 'local_reportbuilder');
        }
        $grouptypeobj = $this->load_grouptype($grouptype);
    ...
    public function get_group_assignedvia_details($userids) {
        ...
        foreach ($assignedgroups as $assignedgroup) {
            ...
            if (!empty($assignedgroup->includechildren)) {
                $string .= get_string('assignincludechildren', 'local_reportbuilder');
            }
            $grouptypeobj = $this->load_grouptype($assignedgroup->grouptype);
...
abstract class totara_assign_core_grouptype {
    ...
    public static function load_grouptype($assignobject) {
        ...
        if (!file_exists($classfile)) {
            print_error('error:assignmentprefixnotfound', 'local_reportbuilder', '', $assignobject->grouptype);
        }
        // Load class file.
        require_once($classfile);
        // Check class exists.
 ...
 */
require_once("$CFG->dirroot/local/reportbuilder/dialogs/dialog_content.class.php");
class totara_assign_ui_picker_cohort extends totara_dialog_content {
    ...
    public function set_parameters($newparams = array()) {
        if (!is_array($newparams)) {
            print_error('error:assignmentbadparameters', 'local_reportbuilder', null, null);
            die();
        }
...
function totara_setup_assigndialogs($module, $itemid, $datatable = false, $notice = "", $suffix = '') {
    ...
    if ($datatable) {
        ...
        $PAGE->requires->js_call_amd('local_reportbuilder/datatable', 'init', $args);
    }
}
