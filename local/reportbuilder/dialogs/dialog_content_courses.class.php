...
require_once($CFG->dirroot.'/local/coursecatalog/lib.php');
...
class totara_dialog_content_courses extends totara_dialog_content {
    ...
    public function load_categories() {
        ...
        if (!$this->categoryid) {
            ...
            if (!$parent = $DB->get_record('course_categories', array('id' => $this->categoryid))) {
                print_error('error:categoryidincorrect', 'local_reportbuilder');
            }
        }
    ...
    public function load_courses($where = false, $userid = 0) {
        ...
        if ($this->categoryid) {
            ...
            if ($where === false) {
                ...
                $this->courses = self::get_course_records($where, $params);
            } else {
     ...
     */
    public static function get_course_records($whereclause, $params) {
        global $DB;
