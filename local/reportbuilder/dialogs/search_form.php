...
class dialog_search_form extends moodleform {
    ...
    function definition() {
        ...
        // Query box
        $query = $this->_customdata['query'];
        $searcharray[] =& $mform->addElement('text', 'query', get_string('search', 'local_reportbuilder'), 'maxlength="254"');
        ...
        if (($searchtype == 'hierarchy' && $showpicker) || !empty($othertree)) {
            ...
            $item->setHiddenLabel(get_string('framework', 'local_reportbuilder'));
            $mform->setDefault('frameworkid', $frameworkid);
        }
