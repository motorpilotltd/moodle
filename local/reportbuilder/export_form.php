...
class report_builder_export_form extends moodleform {
    ...
    function definition() {
        ...
        if (count($select) == 0) {
            ...
            // Show pulldown menu.
            $group=array();
            $group[] =& $mform->createElement('select', 'format', get_string('exportformat', 'local_reportbuilder'), $select);
            $group[] =& $mform->createElement('submit', 'export', get_string('export', 'local_reportbuilder'));
            $mform->addGroup($group, 'exportgroup', get_string('exportas', 'local_reportbuilder'), array(' '), false);
        }
