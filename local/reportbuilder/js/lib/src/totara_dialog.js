    ...
    this.error = function(dialog, response, url) {
        if (response) {
            ...
            require(['core/str'], function (mdlstring) {
                mdlstring.get_string('error:dialoggenericerror', 'local_reportbuilder').done(function (genericerror) {
                    // Hide loading animation.
                    dialog.hideLoading();
            ...
            if (handler._get_ids(elements).length >= handler.selectionlimit) {
                if (!$('.selectionlimiterror').length) {
                    require(['core/templates', 'core/str'], function(templates, mdlstr) {
                        mdlstr.get_string('selectionlimited', 'local_reportbuilder', handler.selectionlimit).then(function(str) {
                            var data = {
                                'announce': 1,
                                'closebutton': 0,
...
totaraDialog_handler_treeview_multiselect_rb_filter.prototype.first_load = function() {
    ...
    $('.multiselect-selected-item', linkcontainer).each(function(i, el) {
        ...
        preselected += '<div class="treeview-selected-item"><span id="item_'+item_id+'"><a href="#">'+item_name+'</a><span class="deletebutton">'
                    + M.util.get_string('delete', 'local_reportbuilder')
                    +'</span></span></div>';
...
totaraDialog_handler_treeview_singleselect.prototype.setup_delete = function() {
    ...
    require(['core/templates'], function (templates) {
        templates.renderPix('t/delete', 'core', M.util.get_string('delete', 'local_reportbuilder')).done(function (html) {
...
totaraDialog_handler_treeview_singleselect.prototype._save = function() {
    ...
    if (selected_val === "0") {
        if (!M.str.hasOwnProperty('local_reportbuilder') || !M.str['local_reportbuilder'].hasOwnProperty('error:'+this._title+'notselected')) {
            require(['core/str'], function (mdlstring) {
                mdlstring.get_string('error:itemnotselected', 'local_reportbuilder').done(function (itemnotselected) {
                    alert(itemnotselected);
                })
            ;})
        } else {
            alert(M.util.get_string('error:'+this._title+'notselected', 'local_reportbuilder'));
        }
        return false;
    }
...
totaraMultiSelectDialog = function(name, title, find_url, save_url) {
    ...
    var buttonObj = {};
    buttonObj[M.util.get_string('save', 'local_reportbuilder')] = function() { handler._save(save_url) };
    buttonObj[M.util.get_string('cancel', 'moodle')] = function() { handler._cancel() };
