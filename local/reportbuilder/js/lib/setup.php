...
function local_js($options = array()) {
    ...
    if (in_array(TOTARA_JS_DIALOG, $options)) {
        ...
        // M.str.COMPONENT.IDENTIFIER, eg; M.str.local_reportbuilder['save']. Can also
        ...
        $PAGE->requires->strings_for_js(array('save', 'delete'), 'local_reportbuilder');
        ...
        $jsmodule = array(
                'name' => 'totara_dialog',
                'fullpath' => '/local/reportbuilder/js/lib/' . $directory . '/totara_dialog' . $min . '.js',
                'requires' => array('json'));
...
function build_treeview($elements, $error_string, $hierarchy = null, $disabledlist = array()) {
    ...
    if (is_array($elements) && !empty($elements)) {
        ...
        if(count($elements) > $maxitems) {
            ...
            $html .= get_string('error:morethanxitemsatthislevel', 'local_reportbuilder', $maxitems);
            $html .= ' <a href="#search-tab" onclick="$(\'#tabs\').tabs(\'select\', 1);return false;">';
            $html .= get_string('trysearchinginstead', 'local_reportbuilder');
            $html .= '</a>';
