...
M.totara_version_tracking = M.totara_version_tracking || {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (currentbranch == BRANCH_UNSUPPORTED) {
                    var message = M.util.get_string('unsupported_branch_text', 'local_reportbuilder', module.config.major_version);
                ...
                if (currentbranch == BRANCH_SUPPORTED && currentrelease != RELEASE_NOT_CURRENT) {
                    var message = M.util.get_string('supported_branch_text', 'local_reportbuilder', module.config.major_version);
                    message = message.replace('[[CURRENT_MAJOR_VERSION]]', current_major_version);
                    module.display_message(message);
                    return true;
                }
                ...
                var message = (alltypes_count == 1) ?
                    M.util.get_string('old_release_text_singular', 'local_reportbuilder') :
                    M.util.get_string('old_release_text_plural', 'local_reportbuilder');
                message = message.replace('[[ALLTYPES_COUNT]]', alltypes_count);
                if (security_count > 0) {
                    message += (security_count == 1) ?
                        M.util.get_string('old_release_security_text_singular', 'local_reportbuilder') :
                        M.util.get_string('old_release_security_text_plural', 'local_reportbuilder');
                    message = message.replace('[[SECURITY_COUNT]]', security_count);
                }

                if (currentbranch == BRANCH_SUPPORTED) {
                    message += M.util.get_string('supported_branch_old_release_text', 'local_reportbuilder', module.config.major_version);
                    message = message.replace('[[CURRENT_MAJOR_VERSION]]', current_major_version);
                }
                module.display_message(message);
                return true;
    ...
    display_message: function(text) {
        text += M.util.get_string('totarareleaselink', 'local_reportbuilder');
