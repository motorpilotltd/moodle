...
$returnurl = new moodle_url('/local/reportbuilder/columns.php', array('id' => $id));
...
if ($h !== null && isset($cid)) {
    if ($report->showhide_column($cid, $h)) {
        \local_reportbuilder\event\report_updated::create_from_report($report, 'columns')->trigger();
        notice(get_string('column_vis_updated', 'local_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
    } else {
        notice(get_string('error:column_vis_not_updated', 'local_reportbuilder'), $returnurl);
    }
}
...
if ($d and $cid) {
    if ($confirm and confirm_sesskey()) {
        if ($report->delete_column($cid)) {
            \local_reportbuilder\event\report_updated::create_from_report($report, 'columns')->trigger();
            notice(get_string('column_deleted', 'local_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
        } else {
            notice(get_string('error:column_not_deleted', 'local_reportbuilder'), $returnurl);
        }
    }
    echo $output->header();

    $confirmurl = new moodle_url('/local/reportbuilder/columns.php', array('d' => '1', 'id' => $id, 'cid' => $cid,
                                                                            'confirm' => '1', 'sesskey' => $USER->sesskey));
    echo $output->confirm(get_string('confirmcolumndelete', 'local_reportbuilder'), $confirmurl, $returnurl);
...
if ($m and $cid and confirm_sesskey()) {
    if ($report->move_column($cid, $m)) {
        \local_reportbuilder\event\report_updated::create_from_report($report, 'columns')->trigger();
        notice(get_string('column_moved', 'local_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
    } else {
        notice(get_string('error:column_not_moved', 'local_reportbuilder'), $returnurl);
    }
}
...
if ($fromform = $mform->get_data()) {
    ...
    if (local_reportbuilder_build_columns($fromform, $report, $allowedadvanced, $grouped)) {
        reportbuilder_set_status($id);
        $report = new reportbuilder($id);
        \local_reportbuilder\event\report_updated::create_from_report($report, 'columns')->trigger();
        notice(get_string('columns_updated', 'local_reportbuilder'), $returnurl, array('class' => 'notifysuccess'));
    } else {
        notice(get_string('error:columns_not_updated', 'local_reportbuilder'), $returnurl);
    }
 ...
 */
function local_reportbuilder_build_columns($fromform, reportbuilder $report, $allowedadvanced, $grouped) {
    global $DB;
