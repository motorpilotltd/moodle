...
require_once($CFG->dirroot . '/local/reportbuilder/classes/rb_global_restriction_set.php');
...
class reportbuilder {
    ...
    public function __construct($id=null, $shortname=null, $embed_deprecated=false, $sid=null, $reportfor=null,
        ...
        if ($id != null) {
            ...
            print_error('noshortnameorid', 'local_reportbuilder');
        }
        ...
        if (!$report) {
            ...
            if ($embed) {
                ...
                if (! $id = reportbuilder_create_embedded_record($shortname, $embed, $error)) {
                    print_error('error:creatingembeddedrecord', 'local_reportbuilder', '', $error);
                }
        ...
        if (!$report) {
            print_error('reportwithidnotfound', 'local_reportbuilder', '', $id);
        }
        ...
        $this->showtotalcount = (!empty($report->showtotalcount) && !empty(get_config('local_reportbuilder', 'allowtotalcount')));
        $this->_sid = $sid;
        ...
        if ($embed) {
            if (defined('REPORTBUIDLER_MANAGE_REPORTS_PAGE')) {
                ...
                require_capability('local/reportbuilder:manageembeddedreports', context_system::instance());
    ...
    public function get_caching_problems() {
        ...
        foreach ($this->filters as $filter) {
            ...
            if (!$filter->is_caching_compatible()) {
                $problems[] = get_string('reportcachingincompatiblefilter', 'local_reportbuilder', $filter->label);
            }
        }
        return $problems;
    }
    ...
    function include_js() {
        ...
        foreach ($this->filters as $filter) {
            ...
            $filterpath = $CFG->dirroot.'/local/reportbuilder/filters/'.$filtertype.'.php';
    ...
    public static function get_source_object($source, $usecache = false, $exception = true, rb_global_restriction_set $globalrestrictionset = null) {
        ...
        $classname = "rbsource_$source\\source";
        ...
        if (preg_match('/^(.+)_grp_([0-9]+)$/', $source, $matches)) {
            ...
            $groupid = $matches[2];
            $classname = "rbsource_$basesource\\source";
            if (class_exists($classname)) {
    ...
    public static function get_source_list($includenonselectable = false) {
        ...
        $plugins = $manager->get_installed_plugins('rbsource');

        foreach ($plugins as $source => $version) {
        ...
        if (is_null($ignored)) {
            ...
            if (!is_array($ignored)) {
                ...
                foreach ($classes as $class) {
                    $emb = new $class();
                    if ($emb->is_ignored()) {
                        preg_match('|^rb_(.*)\_embedded.php$|', $class, $matches);
                        $embedded = $matches[1];
                        $ignored[] = $embedded;
                    }
                }
        ...
        if ($saved = $DB->get_record('report_builder_saved', array('id' => $this->_sid))) {
            ...
            if ($saved->ispublic != 0 || $saved->userid == $this->reportfor) {
                ...
                if (defined('CLI_SCRIPT') && CLI_SCRIPT) {
                    ...
                    print_error('savedsearchnotfoundornotpublic', 'local_reportbuilder');
                }
                return false;
            }
        } else {
        ...
        foreach ($filters as $filter) {
            ...
            if ($filterobj = rb_filter_type::get_filter($type, $value, $advanced, $region, $this)) {
                ...
                if ($filter->customname and $filter->filtername) {
                    ...
                    $filterobj->label = get_string ('headingformat', 'local_reportbuilder', $text);
                }
                $out[$key] = $filterobj;
    ...
    function fetch_sql_filters($extrasql='', $extraparams=array()) {
        ...
        if ($extrasql != '') {
            if (strpos($extrasql, '?')) {
                print_error('extrasqlshouldusenamedparams', 'local_reportbuilder');
            }
            $where_sqls[] = $extrasql;
        }
    ...
    private function get_toolbar_text_filter($toolbarsearchtext) {
        ...
        if ($numberoffields == 0) {
            ...
            return get_string('toolbarsearchtextiscontainedinsingle', 'local_reportbuilder', $a);

        } else {
            $result = get_string('toolbarsearchtextiscontainedinmultiple', 'local_reportbuilder', $toolbarsearchtext);
            $columnnames = array();
            foreach ($columns as $column) {
                $columnobject = self::get_single_item($this->columnoptions, $column->type, $column->value);
    ...
    public function format_column_heading(rb_column $column, $plaintext) {
        if ($column->customheading) {
            ...
            if ($column->grouping === 'none') {
                if ($column->transform) {
                    $heading = get_string("transformtype{$column->transform}_heading", 'local_reportbuilder', $heading);
                } else if ($column->aggregate) {
                    $heading = get_string("aggregatetype{$column->aggregate}_heading", 'local_reportbuilder', $heading);
                }
            }
        }
    ...
    function get_default_headings_array() {
        ...
        foreach ($this->columnoptions as $option) {
            ...
            if (isset($option->addtypetoheading) && $option->addtypetoheading) {
                ...
                $defaultheading = get_string ('headingformat', 'local_reportbuilder', $text);
            }
    ...
    function report_url($params = false) {
        ...
        if ($this->embeddedurl === null) {
            $url = new moodle_url($CFG->wwwroot . '/local/reportbuilder/report.php', array('id' => $this->_id));
        } else {
    ...
    public function display_restrictions() {
        ...
        $jsmodule = array('name' => 'totara_email_scheduled_report',
            'fullpath' => '/local/reportbuilder/js/chooserestriction.js'
        );

        $PAGE->requires->strings_for_js(array('chooserestrictiontitle'), 'local_reportbuilder');
        $PAGE->requires->js_init_call('M.local_reportbuilder_chooserestriction.init', $args, false, $jsmodule);

        $chooselink = html_writer::link('#', get_string('changeglobalrestriction', 'local_reportbuilder'),
        ...
        if (!$appliedids) {
            ...
            $messagehtml = html_writer::div(get_string('selectedglobalrestrictionsselect', 'local_reportbuilder', $chooselink),
        ...
        $chooselink = html_writer::link('#', get_string('changeglobalrestriction', 'local_reportbuilder'), array(
        ...
        $messagehtml = html_writer::div(get_string('selectedglobalrestrictionsmany', 'local_reportbuilder', $a),
            'notifynotice globalrestrictionsnotice alert alert-info');
        echo html_writer::div($messagehtml, 'globalrestrictionscontainer');
    }
    ...
    *
    * @return \local_reportbuilder\rb\access\base[] Array of access plugin classes indexed by type name
    */
    public static function get_all_access_plugins() {
        $plugins = array();
        $classes = core_component::get_component_classes_in_namespace('local_reportbuilder', 'rb\access');
        foreach ($classes as $name => $class) {
            if ($name == 'local_reportbuilder\rb\access\base') {
                continue;
            }

            /** @var local_reportbuilder\rb\access\base $obj */
            $obj = new $name();
            $type = $obj->get_type();
    ...
    */
    public static function get_reports_plugins_access($userid) {
        ...
        /** @var \local_reportbuilder\rb\access\base[] $active_plugins */
    ...
    function get_column_fields($mode = rb_column::REGULAR) {
        ...
        $anyaggregated = false;
        $nonaggregatedcustomfield = false;
        foreach ($this->columns as $column) {
            $anyaggregated = $anyaggregated || isset($column->aggregate);
            $nonaggregatedcustomfield = $nonaggregatedcustomfield || (!isset($column->aggregate) && strpos($column->value, 'custom_field_') === 0);
        }
        ...
        if ($anyaggregated && $nonaggregatedcustomfield) {
            $newfields = [];
            foreach ($fields as $field) {
                if (strpos($field, 'auser.id AS ') === 0) {
                    continue;
                }
                $newfields[] = $field;
            }
    ...
    function get_single_join($join, $usage) {
        ...
        if ($match = $this->get_joinlist_item($join)) {
            ...
            print_error('joinnotinjoinlist', 'local_reportbuilder', '', (object)array('join' => $join, 'usage' => $usage));
            return false;
        }
    }
    ...
    function sort_joins($unsortedjoins) {
        ...
        if (count($items) != 0) {
            print_error('couldnotsortjoinlist', 'local_reportbuilder');
        }
    ...
    function get_full_count() {
        ...
        if (empty($this->_fullcount)) {
            ...
            try {
                ...
                print_error($message, 'local_reportbuilder', $e->getMessage(), $e->debuginfo);
            }
        }
    ...
    function export_data($format) {
        ...
        $format = \local_reportbuilder\tabexport_writer::normalise_format($format);
        if ($format === 'fusion') {
            $this->download_fusion(); // Redirect.
            die;
        }

        $formats = \local_reportbuilder\tabexport_writer::get_export_classes();
        ...
        $source = new \local_reportbuilder\tabexport_source($this);

        /** @var \local_reportbuilder\tabexport_writer $writer */
        $writer = new $writerclass($source);

        // Log export event.
        \local_reportbuilder\event\report_exported::create_from_report($this, $format)->trigger();
    ...
    public function display_table($return = false) {
        ...
        if (count($columns) == 0) {
            $html = html_writer::tag('p', get_string('error:nocolumnsdefined', 'local_reportbuilder'));
            if ($return) {
                return $html;
            }
            echo $html;
        ...
        $graph = new \local_reportbuilder\local\graph($this);
        if (!$graph->is_valid()) {
            $graph = null;
        }
        ...
        if ($cacheschedule) {
            ...
            $html .= get_string('report:cachelast', 'local_reportbuilder', $lastreport);
            $html .= html_writer::empty_tag('br');
            $html .= get_string('report:cachenext', 'local_reportbuilder', $nextreport);
        ...
        if ($initiallyhidden) {
            ...
            $table->set_no_records_message(get_string('initialdisplay_pending', 'local_reportbuilder'));
        } else {
            ...
            $records = \local_reportbuilder\dblib\base::getbdlib()->get_counted_recordset_sql($sql . $order, $params, $pagestart, $perpage);

            $table->pagesize($perpage, $this->get_filtered_count());
            $table->add_toolbar_pagination('right', 'both');
            if ($this->is_report_filtered()) {
                $table->set_no_records_message(get_string('norecordswithfilter', 'local_reportbuilder'));
            } else {
                $table->set_no_records_message(get_string('norecordsinreport', 'local_reportbuilder'));
            }
        ...
        if ($graph and $graphdata = $graph->fetch_svg()) {
            if (core_useragent::check_browser_version('MSIE', '6.0') and !core_useragent::check_browser_version('MSIE', '9.0')) {
                ...
                $svgurl = new moodle_url('/local/reportbuilder/ajax/graph.php', array('id' => $this->_id, 'sid' => $this->_sid));
                if ($this->globalrestrictionset) {
                ...
                $nopdf = get_string('error:nopdf', 'local_reportbuilder');
    ...
    function download_fusion() {
        $jump = new moodle_url('/local/reportbuilder/fusionexporter.php', array('id' => $this->_id, 'sid' => $this->_sid));
        redirect($jump->out());
        die;
    }
    ...
    public function get_all_filters_select() {
        ...
        $allstandardfilters = array_merge(
                array(get_string('new') => array(0 => get_string('addanotherfilter', 'local_reportbuilder'))),
        ...
        $allsearchcolumns = array_merge(
            array(get_string('new') => array(0 => get_string('addanothersearchcolumn', 'local_reportbuilder'))),
    ...
    function delete_column($cid) {
        ...
        if ($graphseries) {
            ...
            if (in_array($source, $datasources)) {
                notice(get_string('error:graphdeleteseries', 'local_reportbuilder'));
                return false;
            }
        }
    ...
    function print_feedback_results() {
        ...
        if ($this->is_initially_hidden()) {
            return get_string('initialdisplay_pending', 'local_reportbuilder');
        }
        ...
        if (!$groupid) {
            $out .= get_string('activitygroupnotfound', 'local_reportbuilder');
        }
        ...
        foreach ($data as $item) {
            ...
            $out .= html_writer::tag('p', get_string('resultsfromfeedback', 'local_reportbuilder', $item->responses_number));
    ...
    function get_feedback_standard_answer($qid, $item) {
        ...
        $answer = 'q' . $qid . '_list';
        if (isset($item->$count)) {
            $out .= html_writer::tag('p', get_string('numresponses', 'local_reportbuilder', $item->$count));
        }
    ...
    function get_feedback_option_answer($qid, $options, $item) {
        ...
        if (isset($item->$numresp)) {
            $out .= html_writer::tag('p', get_string('numresponses', 'local_reportbuilder', $item->$numresp));
        }
    ...
    public function post_config_visibility_where($type, $table, $userid = null, $showhidden = false, $fieldid = 'id',
        ...
        return totara_visibility_where($userid, $vis, $type, $this->is_cached(), $showhidden);
    }
...
require_once($CFG->dirroot . '/local/reportbuilder/additionallib.php');
        ...
        case 'mssql':
            ...
            $columnssql = "SELECT sc.name, sc.system_type_id, sc.max_length, st.name as field_type FROM sys.columns sc
                    LEFT JOIN sys.types st ON (st.system_type_id = sc.system_type_id
                        AND st.name <> 'sysname' AND st.name <> 'geometry')
...
function sql_group_concat($field, $delimiter=', ', $unique=false) {
    ...
    if ($unique) {
        return \local_reportbuilder\dblib\base::getbdlib()->sql_group_concat_unique($field, $delimiter);
    } else {
        return \local_reportbuilder\dblib\base::getbdlib()->sql_group_concat($field, $delimiter);
    }
}
...
function reportbuilder_send_scheduled_report($sched) {
    ...
    if ($sched->savedsearchid != 0) {
        $messagedetails->savedtext = get_string('savedsearchmessage', 'local_reportbuilder', $saved->name);
    } else {
    ...
    if ($sched->exporttofilesystem == REPORT_BUILDER_EXPORT_EMAIL
        ...
        foreach ($externalusers as $email) {
            $userto = get_external_user($email);
            $result = email_to_user($userto, $fromaddress, $subject, $messageplain, $messagehtml, $tempfile, $attachmentfilename);
            if ($result) {
                $emailedcount++;
            } else {
...
function get_external_user($emailaddress) {
    global $CFG;

    static $externaluser = null;

    // We haven't got an external user in cache, better create one.
    if (empty($externaluser)) {
        $externaluser = core_user::get_noreply_user();
        $externaluser->username = "external";
        $externaluser->lang = $CFG->lang;
        $externaluser->maildisplay = 1;
        $externaluser->emailstop = 0;
        $externaluser->mailformat = 1;
    }

    // Set some fields for this external user object.
    $externaluser->email = $emailaddress;
    $externaluser->firstname = $emailaddress;
    return $externaluser;
}
 ...
 * @param string $writerclass class extending \local_reportbuilder\tabexport_writer
 *
...
function reportbuilder_export_schduled_report(stdClass $sched, reportbuilder $report, $writerclass) {
    ...
    $source = new \local_reportbuilder\tabexport_source($report);
...
function reportbuilder_get_all_scheduled_reports_without_recipients() {
    ...
    $sql = '
      ...
      LEFT JOIN {report_builder_schd_eml_aud} rbsea ON (rbsea.scheduleid = rbs.id)
      LEFT JOIN {report_builder_schd_eml_ext} rbsee ON (rbsee.scheduleid = rbs.id)
      LEFT JOIN {report_builder_schd_eml_user} rbses ON (rbses.scheduleid = rbs.id)
...
function reportbuilder_get_export_filename($report, $userid, $scheduleid) {
    ...
    $reportfilename = format_string($report->fullname) . '_' .
            userdate(time(), get_string('datepickerlongyearphpuserdate', 'local_reportbuilder')) . '_' . $scheduleid;
    ...
    if (!empty($path)) {
        ...
        if (preg_match($pattern, $path)) {
            mtrace(get_string('error:notapathexportfilesystempath', 'local_reportbuilder'));
        } else if (!is_dir($path)) {
            mtrace(get_string('error:notdirexportfilesystempath', 'local_reportbuilder'));
        } else if (!is_writable($path)) {
            mtrace(get_string('error:notwriteableexportfilesystempath', 'local_reportbuilder'));
        }
    }
...
function reportbuilder_create_embedded_record($shortname, $embed, &$error) {
    ...
    try {
        ...
        //\local_reportbuilder\event\report_created::create_from_report($report, true)->trigger();
...
function rb_unique_param($name) {
    global $DB;
    return \local_reportbuilder\dblib\base::getbdlib()->get_unique_param($name);
}
...
function reportbuilder_get_export_options($currentoption = null, $includefusion = false) {
    ...
    foreach ($exportoptions as $option) {
        $option = \local_reportbuilder\tabexport_writer::normalise_format($option);
        if ($option) {
            $enabled[$option] = true;
        }
    }
    ...
    $select = array();
    $alloptions = \local_reportbuilder\tabexport_writer::get_export_options();
    ...
    if ($includefusion and isset($enabled['fusion'])) {
        $select['fusion'] = get_string('exportfusion', 'local_reportbuilder');
    }
...
*/
function local_reportbuilder_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, $options=array()) {
    $fs = get_file_storage();
    $relativepath = implode('/', $args);
    $fullpath = "/{$context->id}/local_reportbuilder/$filearea/$args[0]/$args[1]";
...
function reportbuilder_delete_report($id) {
    ...
    $DB->delete_records_select('report_builder_schd_eml_aud', $select, array($id));
    $DB->delete_records_select('report_builder_schd_eml_user', $select, array($id));
    $DB->delete_records_select('report_builder_schd_eml_ext', $select, array($id));
...
function reportbuilder_set_default_access($reportid) {
    ...
    $acess = new \local_reportbuilder\rb\access\role();
...
function local_reportbuilder_extend_navigation($navigation) {
    global $USER, $DB, $PAGE;
    ...
    $reports = get_string('reports', 'local_reportbuilder');
    $nodemymedia = $nodehome->add($reports, new moodle_url('/local/reportbuilder/myreports.php'), navigation_node::NODETYPE_LEAF, $reports, 'reports');
    $nodemymedia->showinflatnavigation = true;
}

require_once("$CFG->dirroot/lib/formslib.php");
\MoodleQuickForm::registerElementType('scheduler',
        $CFG->dirroot . '/local/reportbuilder/classes/form/scheduler.php',
        '\\local_reportbuilder\\form\\scheduler');