 ...
 */
M.local_reportbuildercolumns = M.local_reportbuildercolumns || {
    ...
    init: function(Y, config){
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.local_reportbuildercolumns.init()-> jQuery dependency required for this module to function.');
        }
        ...
        require(['core/templates'], function (templates) {
            ...
            iconscache.push(templates.renderPix('i/hide', 'core', M.util.get_string('hide', 'local_reportbuilder')));
            iconscache.push(templates.renderPix('i/show', 'core', M.util.get_string('show', 'local_reportbuilder')));
        ...
        if (colName == '0') {
            ...
            if (advancedSelectorVal != '') {
                if ($.inArray(advancedSelectorVal, module.config.rb_allowed_advanced[colName]) == -1) {
                    ...
                    newHeading = M.util.get_string(strName, 'local_reportbuilder', newHeading);
                }
            }
    ...
    rb_init_addbutton: function(colselector){
        ...
        addbutton.on('click', function(e) {
            ...
            e.preventDefault();
            $.ajax({
                url: M.cfg.wwwroot + '/local/reportbuilder/ajax/column.php',
                type: "POST",
                data: data,
                beforeSend: function() {
    ...
    rb_init_deletebuttons: function() {
        ...
        $('.reportbuilderform table .deletecolbtn').on('click', function(e) {
            ...
            confirmed = confirm(M.util.get_string('confirmcoldelete', 'local_reportbuilder'));
    ...
    rb_get_btn_hide: function(reportid, colid) {
        return $('<a href="' + M.cfg.wwwroot + '/local/reportbuilder/columns.php?id=' + reportid + '&cid='+colid+'&h=1" title="' + M.util.get_string('hide', 'local_reportbuilder') + '" class="hidecolbtn action-icon">' + this.hideicon +'</a>');
    },

    rb_get_btn_show: function(reportid, colid) {
        return $('<a href="' + M.cfg.wwwroot + '/local/reportbuilder/columns.php?id=' + reportid + '&cid='+colid+'&h=0" title="' + M.util.get_string('show', 'local_reportbuilder') + '" class="showcolbtn action-icon">' + this.showicon + '</a>');
    },

    rb_get_btn_delete: function(reportid, colid) {
        return $('<a href="' + M.cfg.wwwroot + '/local/reportbuilder/columns.php?id=' + reportid + '&cid='+colid+'&d=1" title="' + M.util.get_string('delete', 'local_reportbuilder') + '" class="deletecolbtn action-icon">' + this.deleteicon + '</a>');
    },

    rb_get_btn_up: function(reportid, colid) {
        return $('<a href="' + M.cfg.wwwroot + '/local/reportbuilder/columns.php?id=' + reportid + '&cid='+colid+'&m=up" title="' + M.util.get_string('moveup', 'local_reportbuilder') + '" class="movecolupbtn action-icon">' + this.upicon + '</a>');
    },

    rb_get_btn_down: function(reportid, colid) {
        return $('<a href="' + M.cfg.wwwroot + '/local/reportbuilder/columns.php?id=' + reportid + '&cid='+colid+'&m=down" title="' + M.util.get_string('movedown', 'local_reportbuilder') + '" class="movecoldownbtn action-icon">' + this.downicon + '</a>');
    },

    rb_get_btn_add: function(reportid) {
        return $('<a href="' + M.cfg.wwwroot + '/local/reportbuilder/columns.php?id=' + reportid + '" class="addcolbtn"><input type="button" value="' + M.util.get_string('add', 'local_reportbuilder') + '" /></a>');
    }
};
