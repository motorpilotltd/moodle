...
$continueurl = new moodle_url('/local/reportbuilder/restrictions/edit_restrictedusers.php', array('id' => $id));
...
if ($restriction->allusers) {
    ...
    echo html_writer::tag('p', get_string('restrictionallusers', 'local_reportbuilder'));

    $allusersurl = new moodle_url('/local/reportbuilder/restrictions/edit_restrictedusers.php',
        array('id' => $id, 'allusers' => 0, 'sesskey' => sesskey()));
    echo $output->single_button($allusersurl, get_string('restrictiondisableallusers', 'local_reportbuilder'));
...
echo $output->edit_restriction_header($restriction, 'restrictedusers');
echo html_writer::tag('p', get_string('restrictedusersdescription', 'local_reportbuilder'));

$allusersurl = new moodle_url('/local/reportbuilder/restrictions/edit_restrictedusers.php',
    array('id' => $id, 'allusers' => 1, 'sesskey' => sesskey()));
echo $output->single_button($allusersurl, get_string('restrictionenableallusers', 'local_reportbuilder'));
