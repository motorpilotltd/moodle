...
$assign = new totara_assign_reportbuilder_record('reportbuilder', $restriction, 'record');
$grouptypes = $assign->get_assignable_grouptype_names();
$grouptypes = array_merge(array("" => get_string('assigngroup', 'local_reportbuilder')), $grouptypes);

$continueurl = new moodle_url('/local/reportbuilder/restrictions/edit_recordstoview.php', array('id' => $id));
...
if ($restriction->allrecords) {
    ...
    echo html_writer::tag('p', get_string('restrictionallrecords', 'local_reportbuilder'));

    $allrecordsurl = new moodle_url('/local/reportbuilder/restrictions/edit_recordstoview.php',
        array('id' => $id, 'allrecords' => 0, 'sesskey' => sesskey()));
    echo $output->single_button($allrecordsurl, get_string('restrictiondisableallrecords', 'local_reportbuilder'));
...
echo html_writer::tag('p', get_string('recordstoviewdescription', 'local_reportbuilder'));

$allrecordsurl = new moodle_url('/local/reportbuilder/restrictions/edit_recordstoview.php',
    array('id' => $id, 'allrecords' => 1, 'sesskey' => sesskey()));
echo $output->single_button($allrecordsurl, get_string('restrictionenableallrecords', 'local_reportbuilder'));

echo $output->heading(get_string('assignedgroups', 'local_reportbuilder'), 3);
...
echo $output->heading(get_string('assignedusers', 'local_reportbuilder'), 3);
