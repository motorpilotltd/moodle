...
class rb_filter_multicheck extends rb_filter_type {
    ...
    function setupForm(&$mform) {
        ...
        if (count($options) == 0) {
            $mform->addElement('static', $this->name, $label, get_string('nofilteroptions', 'local_reportbuilder'));
            if ($advanced) {
        ...
        $mform->addHelpButton($this->name . '_grp', 'filtercheckbox', 'local_reportbuilder');
        ...
        foreach ($this->options['selectchoices'] as $option => $unused) {
            ...
            foreach ($otherfilters as $otherfilter) {
                if ($otherfilter != $this) {
                    ...
                    $inactiveuniqueparam = \local_reportbuilder\dblib\base::getbdlib()->get_unique_param('filter');
            ...
            $filtercountuniqueparam = \local_reportbuilder\dblib\base::getbdlib()->get_unique_param('filter_count');
