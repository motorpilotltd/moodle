...
class rb_filter_date extends rb_filter_type {
    ...
    function setupForm(&$mform) {
        ...
        $objs[] =& $mform->createElement('checkbox', $this->name.'_sck', null, get_string('isafter', 'local_reportbuilder'));
        if ($includetime) {
        ...
        $objs[] =& $mform->createElement('static', null, null, html_writer::empty_tag('br'));
        $objs[] =& $mform->createElement('checkbox', $this->name.'_eck', null, get_string('isbefore', 'local_reportbuilder'));
        if ($includetime) {
        ...
        if ($includebetween) {
            ...
            $objs['pre'] =& $mform->createElement('checkbox', $this->name.'daysbeforechkbox', null,
                    get_string('dateisbetween', 'local_reportbuilder'));
            $accesslabel = html_writer::tag(
                'label',
                get_string('isbeforetoday', 'local_reportbuilder'),
                array('class' => 'accesshide', 'for' => 'id_' . $objs['pre']->getAttribute('name'))
            );
            $objs[] =& $mform->createElement('static', null, null, $accesslabel);
            $objs[] =& $mform->createElement('text', $this->name.'daysbefore', get_string('isbeforetoday', 'local_reportbuilder'), 'size="2"');
            $mform->setType($this->name.'daysbefore', PARAM_INT);
            $objs[] =& $mform->createElement('static', null, null, html_writer::span(get_string('isbeforetoday', 'local_reportbuilder')));
            $objs[] =& $mform->createElement('static', null, null, html_writer::empty_tag('br'));
            $objs['post'] =& $mform->createElement('checkbox', $this->name.'daysafterchkbox', null,
                    get_string('dateisbetween', 'local_reportbuilder'));
            $accesslabel = html_writer::tag(
                'label',
                get_string('isaftertoday', 'local_reportbuilder'),
                array('class' => 'accesshide', 'for' => 'id_' . $objs['post']->getAttribute('name'))
            );
            $objs[] =& $mform->createElement('static', null, null, $accesslabel);
            $objs[] =& $mform->createElement('text', $this->name.'daysafter', get_string('isaftertoday', 'local_reportbuilder'), 'size="2"');
            $mform->setType($this->name.'daysafter', PARAM_INT);
            $objs[] =& $mform->createElement('static', null, null, html_writer::span(get_string('isaftertoday', 'local_reportbuilder')));
        }

        if ($includenotset) {
            $objs[] =& $mform->createElement('static', null, null, html_writer::empty_tag('br'));
            $objs[] =& $mform->createElement('checkbox', $this->name.'notset', null, get_string('datenotset', 'local_reportbuilder'));
        }

        $grp =& $mform->addElement('group', $this->name.'_grp', $label, $objs, '', false);
        $mform->addHelpButton($grp->_name, 'filterdate', 'local_reportbuilder');
    ...
    function get_label($data) {
        ...
        if ($label) {
            return get_string($label, 'local_reportbuilder', $a);
        } else {
            return '';
        }
    }
