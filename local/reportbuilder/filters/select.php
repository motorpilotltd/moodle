...
class rb_filter_select extends rb_filter_type {
    ...
    function setupForm(&$mform) {
        ...
        if ($simplemode) {
            // simple select mode
            $choices = array('' => get_string('anyvalue', 'local_reportbuilder')) + $options;
            $mform->addElement('select', $this->name, $label, $choices, $attr);
            $mform->setType($this->name, PARAM_TEXT);
            $mform->addHelpButton($this->name, 'filtersimpleselect', 'local_reportbuilder');
            if ($advanced) {
            ...
            $mform->addHelpButton($grp->_name, 'filterselect', 'local_reportbuilder');
    ...
    function get_label($data) {
        ...
        if ($simplemode) {
            ...
            $a->label    = $label;
            $a->value    = '"' . s($options) . '"';
            $a->operator = get_string('isequalto', 'local_reportbuilder');
