...
class rb_filter_type {
    ...
    protected function get_filteroption($type, $value) {
        ...
        if (!isset($this->report->filteroptions[$key])) {
            ...
            debugging(get_string('error:filteroptiontypexandvalueynotfoundinz', 'local_reportbuilder', $a), DEBUG_DEVELOPER);
            return false;
        }
    ...
    protected function get_columnoption($type, $value) {
        ...
        if (!$columnoption) {
            ...
            $a->value = $value;
            $a->source = $sourcename;
            debugging(get_string('error:columnoptiontypexandvalueynotfoundinz', 'local_reportbuilder', $a), DEBUG_DEVELOPER);
            return false;
        }
    ...
    protected function get_joins($filteroption, $columnoption) {
        ...
        if (!reportbuilder::check_joins($this->report->src->joinlist, $joins)) {
            ...
            $a->source = get_class($this->report->src);
            debugging(get_string('error:joinsforfiltertypexandvalueynotfoundinz', 'local_reportbuilder', $a), DEBUG_DEVELOPER);
            return false;
        }
    ...
    public static function get_filter($type, $value, $advanced, $region, $report) {
        ...
        if (!class_exists($classname)) {
            $filename = "{$CFG->dirroot}/local/reportbuilder/filters/{$filtertype}.php";
    ...
    function get_sql_filter($data) {
        print_error('abstractmethodcalled', 'local_reportbuilder', '', 'get_sql_filter()');
    }
    ...
    function check_data($formdata) {
        print_error('abstractmethodcalled', 'local_reportbuilder', '', 'check_data()');
    }
    ...
    function setupForm(&$mform) {
        print_error('abstractmethodcalled', 'local_reportbuilder', '', 'setupForm()');
    }
    ...
    function get_label($data) {
        print_error('abstractmethodcalled', 'local_reportbuilder', '', 'get_label()');
    }
...
function search_get_keyword_where_clause($field, $keywords, $negate=false, $operator='contains') {
    ...
    foreach ($keywords as $keyword) {
        $uniqueparam = \local_reportbuilder\dblib\base::getbdlib()->get_unique_param('search');
