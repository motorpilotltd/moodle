 ...
 *
 * Copyright (C) 2018 onwards T0tara Learning Solutions LTD
 *
...
class rb_filter_system_role extends rb_filter_type {
    ...
    public function get_label($data) {
        ...
        if ($data['operator'] && $data['value'] == '0') {
            $label = get_string($operator . 'anyrole', 'local_reportbuilder');
        } else if ($data['operator'] && !empty($data['value'])) {
            $label = get_string($operator . 'role', 'local_reportbuilder', array('role' => $options[$data['value']]));
        } else {
            $label = get_string('noroleselected' ,'local_reportbuilder');
        }
    ...
    private function get_operator_options() {
        return array(
            self::OPERATOR_ASSIGNED => get_string('assigned', 'local_reportbuilder'),
            self::OPERATOR_NOTASSIGNED => get_string('notassigned', 'local_reportbuilder')
        );
    }
