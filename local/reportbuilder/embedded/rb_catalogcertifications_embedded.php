 ...
 */
require_once($CFG->dirroot.'/local/reportbuilder/embedded/rb_findcertifications_embedded.php');

class rb_catalogcertifications_embedded extends rb_base_embedded {
    public function __construct($data) {
        $this->url = '/local/coursecatalog/certifications.php';
        $this->source = 'certification';
        $this->shortname = 'catalogcertifications';
        $this->fullname = get_string('catalogcertifications', 'local_coursecatalog');

        $this->columns = array(
            array(
                'type' => 'certif',
                'value' => 'certifexpandlink',
                'heading' => get_string('certifname', 'local_custom_certification')
            ),
            array(
                'type' => 'certif',
                'value' => 'summary',
                'heading' => get_string('certificationsummary', 'local_custom_certification')
            )
        );
    ...
    public function get_extrabuttons() {
        ...
        if ($categoryid !== false) {
            ...
            $createbutton = new single_button($createurl, get_string('addcertification', 'local_coursecatalog'), 'get');
