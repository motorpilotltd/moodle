...
class backup_threesixty_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $threesixty = new backup_nested_element(
            'threesixty',
            array('id'),
            array(
                'name', 'intro', 'introformat',
                'requiredrespondents', 'respondentselection', 'allowexternalrespondents', 'respondenttypes', 'selftypes',
                'grade', 'skillgrade',
                'questionorder', 'competencylabels',
                'competencyalternative', 'skillalternative',
                'reportsavailable',
        ...
        $competency = new backup_nested_element('competency', array('id'),
            array('name', 'description', 'colour', 'showfeedback', 'sortorder')
        );

        $skills = new backup_nested_element('skills');

        $skill = new backup_nested_element('skill', array('id'),
            array('name', 'description', 'altname', 'altdescription', 'sortorder')
        );

        $analyses = new backup_nested_element('analyses');

        $analysis = new backup_nested_element('analysis', array('id'),
            array('userid')
        );

        $respondents = new backup_nested_element('respondents');

        $respondent = new backup_nested_element('respondent', array('id'),
            array('userid', 'respondentuserid', 'firstname', 'lastname', 'email', 'type', 'uniquehash', 'declined', 'declinetime')
        );

        $responses = new backup_nested_element('responses');

        $response = new backup_nested_element('response', array('id'),
            array('timecompleted')
        );

        $responseskills = new backup_nested_element('responseskills');

        $responseskill = new backup_nested_element('responseskill', array('id'),
            array('skillid', 'score')
        );

        $responsecomps = new backup_nested_element('responsecomps');

        $responsecomp = new backup_nested_element('responsecomp', array('id'),
            array('competencyid', 'feedback', 'feedbackformat')
        );
        ...
        $threesixty->add_child($analyses);
        $analyses->add_child($analysis);
        ...
        if ($userinfo) {
            ...
            $response->set_source_table('threesixty_response', array('respondentid' => backup::VAR_PARENTID));
