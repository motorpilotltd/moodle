...
$code = optional_param('code', '', PARAM_ALPHANUM); // unique hash for external users
...
if (!$externalrespondent) {
    ...
    if (!$user = $DB->get_record('user', array('id' => $userid), 'id, firstname, lastname')) {
        print_error('error:invaliduserid', 'threesixty');
    }
    if ($analysis = $DB->get_record('threesixty_analysis', array('userid' => $user->id, 'activityid' => $a))) {
...
$PAGE->set_cm($cm);
$PAGE->set_pagelayout('incourse'); // Needed for external respondents to see blocks.

$questionorder = $threesixty->questionorder;

if ($page < 1 && $questionorder == 'competency') {
...
$locked = false;
if ($questionorder == 'competency') {
    if ($competency = get_paginated_competency_details($page, $threesixty->id, $user->id, $respondent)) {
        $locked = $competency->locked;
        // one competency sheet per page
        $mform = new mod_threesixty_paginated_score_form(null, compact('a', 'code', 'competency', 'page', 'nbpages', 'userid', 'typeid'));
        if ($mform->is_cancelled()){
            redirect($cancelurl);
        }
        if ($fromform = $mform->get_data()) {
            if (!empty($fromform->buttonarray['previous'])) { // Previous button
                $errormsg = save_paginated_changes($fromform, $threesixty->id, $user->id, $competency, false, $respondent);
                if (!empty($errormsg)) {
            ...
            elseif (!empty($fromform->buttonarray['finish'])) {
                $errormsg = save_paginated_changes($fromform, $threesixty->id, $user->id, $competency, true, $respondent);
                if (!empty($errormsg)) {
                    print_error('error:cannotsavescores', 'threesixty', $currenturl, $errormsg);
                }
                if (!$externalrespondent || $internalbypass) {
                    $redirecturl = new moodle_url('/mod/threesixty/view.php', array('a' => $threesixty->id, 'userid' => $user->id));
                    redirect($redirecturl);
                } else {
                    $redirecturl = new moodle_url('/mod/threesixty/thankyou.php', array('a' => $threesixty->id));
                    redirect($redirecturl);
                }
            } else {
    ...
    // All on one page
    list($competencies, $competenciescopy, $locked) = get_single_page_competency_details($threesixty->id, $user->id, $respondent);
    if ($competencies) {
        $mform = new mod_threesixty_single_page_score_form(null, compact('a', 'code', 'competenciescopy', 'locked', 'questionorder', 'userid', 'typeid'));
        if ($mform->is_cancelled()){
            redirect($cancelurl);
        }
        if ($fromform = $mform->get_data()) {
            if (!empty($fromform->buttonarray['finish'])) {
                $errormsg = save_single_page_changes($fromform, $threesixty->id, $user->id, $competencies, $locked, true, $respondent);
                if (!empty($errormsg)) {
                    print_error('error:cannotsavescores', 'threesixty', $currenturl, $errormsg);
                }
                if (!$externalrespondent || $internalbypass) {
        ...
        print_error('error:nocompetencies', 'threesixty');
    }
}

$event = \mod_threesixty\event\scoring_viewed::create(array(
    'context' => $context,
...
$fakeblock = new block_contents(array('id' => 'threesixty_score_page_block', 'class' => 'block'));
$fakeblock->collapsible = block_contents::VISIBLE;
$fakeblock->title = get_string('page:score:block:title', 'threesixty');
$fakeblock->content = get_string('page:score:block:content', 'threesixty');
...
if (!$externalrespondent) {
    ...
    echo get_string('page:score:header', 'threesixty');
} else {
    $RESPONDENTTYPES = threesixty_get_respondent_types($threesixty);
    $m = new StdClass;
    $m->respondent = format_string($respondent->email);
    $m->respondentfullname = $respondent->firstname.' '.$respondent->lastname;
...
if ($mform) {
    switch ($questionorder) {
        case 'competency':
            set_paginated_form_data($mform, $competency);
            break;
        default:
            set_single_page_form_data($mform, $competencies);
            break;
    }
    $mform->display();
} else {
    echo $OUTPUT->box(get_string('nocompetencies', 'threesixty', core_text::strtolower(threesixty_get_alternative_word($threesixty, 'competency', 'competencies'))));
}

echo get_string('page:score:footer', 'threesixty');
...
function get_paginated_competency_details($page, $activityid, $userid, $respondent){
    global $CFG, $DB;
    ...
    if ($competencies = $DB->get_records('threesixty_competency', array('activityid' => $activityid), 'sortorder', '*', $page - 1, 1)) {
        ...
        $record->skills = $DB->get_records('threesixty_skill', array('competencyid' => $record->id), 'sortorder', 'id, name, description, altname, altdescription');
...
function get_single_page_competency_details($activityid, $userid, $respondent){
    global $DB;

    $competencies = $DB->get_records('threesixty_competency', array('activityid' => $activityid), 'sortorder');
    $competenciescopy = array();
    $locked = false;
    foreach ($competencies as $competency) {
        ...
        $response = $DB->get_record_sql($responsesql, array($competency->id, $userid, $activityid));

        if ($response and !empty($response->competencyfeedback)) {
            $competency->feedback = $response->competencyfeedback;
        }

        $competency->locked = false;
        if ($response and !empty($response->timecompleted)) {
            $locked = $competency->locked = true;
        }

        // Get skill descriptions
        $competency->skills = $DB->get_records('threesixty_skill', array('competencyid' => $competency->id), 'sortorder', 'id, name, description, altname, altdescription');

        if ($competency->skills and $response and $response->responseid != null) {
            // Get scores
            $sql = "
                SELECT
                    s.id,
            ...
            if ($scores = $DB->get_records_sql($sql, array($competency->id, $response->responseid))) {
                foreach ($scores as $s) {
                    $competency->skills[$s->id]->score = $s->score;
                }
            }
        }
        $competenciescopy[$competency->id] = clone($competency);
    }
    return array($competencies, $competenciescopy, $locked);
}

function set_paginated_form_data($mform, $competency){
    $toform = array();
    ...
    if (!empty($competency->skills) and count($competency->skills) > 0) {
        foreach ($competency->skills as $skill) {
            if (isset($skill->score)) {
                $toform["score_{$skill->id}"] = $skill->score;
            }
        }
    }
...
function set_single_page_form_data($mform, $competencies){
    $toform = array();
    ...
    foreach ($competencies as $competency) {
        if (!empty($competency->skills) and count($competency->skills) > 0) {
            foreach ($competency->skills as $skill) {
                if (isset($skill->score)) {
                    $toform["score[{$competency->id}][{$skill->id}]"] = $skill->score;
                }
            }
        }
    }
...
function save_paginated_changes($formfields, $activityid, $userid, $competency, $finished, $respondent){
    global $DB;
    ...
    if ($respondent == null) {
        ...
        $respondent->activityid = $analysis->activityid;
...
function save_single_page_changes($formfields, $activityid, $userid, $competencies, $locked, $finished, $respondent){
    global $DB;
    ...
    foreach ($competencies as $competency) {
        foreach ($competency->skills as $skill) {
            $scorevalue = 0;
            if (!empty($formfields->score[$competency->id][$skill->id])) {
                $scorevalue = $formfields->score[$competency->id][$skill->id];
            }
