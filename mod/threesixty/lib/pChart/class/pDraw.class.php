 ...
 */
 ...
 /* BEGIN CORE MOD */
 define("LEGEND_HORIZONTAL_FLIPPED"	, 690903);
 /* END CORE MOD */
   ...
   function drawLegend($X,$Y,$Format="")
    {
     ...
     $Mode	= isset($Format["Mode"]) ? $Format["Mode"] : LEGEND_VERTICAL;
/* BEGIN CORE MOD */
     $ModeFlipped = isset($Format["ModeFlipped"]) ? $Format["ModeFlipped"] : FALSE;
     $leftLegendBoundary = $X;
/* END CORE MOD */
     ...
     foreach($Data["Series"] as $SerieName => $Serie)
      {
       if ( $Serie["isDrawable"] == TRUE && $SerieName != $Data["Abscissa"] )
        {
/* BEGIN CORE MOD */
/* Test label length before commiting... */
         if ( $Mode == LEGEND_HORIZONTAL )
          {
           $TestWidth = 0;
           foreach($Lines as $Key => $Value)
            {
             $TestPos = $this->getTextBox($X+$IconAreaWidth+4,$Y+$IconAreaHeight/2+(($this->FontSize+3)*$Key),$FontName,$FontSize,0,$Value);
             $TestWidth = max(array($TestWidth, $TestPos[1]["X"]));
            }
            if($TestWidth+2+$XStep > $this->GraphAreaX2)
             {
              $X = $leftLegendBoundary;
              if ($ModeFlipped == TRUE)
               {
                $Y -= 2*($IconAreaHeight + $this->FontSize);
               }
               else
               {
                $Y += 2*($IconAreaHeight + $this->FontSize);
...
/* BEGIN CORE MOD */
             $TestPos = $this->getTextBox($X+$IconAreaWidth+4,$Y+$IconAreaHeight/2+(($this->FontSize+3)*$Key),$FontName,$FontSize,0,$Value);
             $YPos = ($Y+$IconAreaHeight/2+(($this->FontSize+3)*$Key) + $TestPos[TEXT_ALIGN_BOTTOMLEFT]['Y']) / 2;
             $BoxArray = $this->drawText($X+$IconAreaWidth+4,$YPos,$Value,array("R"=>$FontR,"G"=>$FontG,"B"=>$FontB,"Align"=>TEXT_ALIGN_MIDDLELEFT,"FontSize"=>$FontSize,"FontName"=>$FontName));
/* END CORE MOD */
             $Width[] = $BoxArray[1]["X"];
            }
           $X=max($Width)+2+$XStep;
