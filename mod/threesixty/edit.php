...
$event = \mod_threesixty\event\administration_viewed::create(array(
    'context' => $context,
...
if (count($competencies) > 0) {
    ...
    $skillsstr = threesixty_get_alternative_word($threesixty, 'skill', 'skills');
    $feedbackstr = get_string('feedback', 'threesixty');
    $actionsstr = get_string('actions', 'threesixty');

    $table = new html_table();
    $table->width = '100%';
    $table->head = array($namestr, $descriptionstr, $skillsstr, $feedbackstr, $actionsstr);
...
echo '<p><a href="editcompetency.php?a='.$threesixty->id.'&amp;c=0">'.get_string('addnewcompetency', 'threesixty', core_text::strtolower(threesixty_get_alternative_word($threesixty, 'competency'))).'</a></p>';
