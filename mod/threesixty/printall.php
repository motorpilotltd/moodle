...
$records = threesixty_users($threesixty, $groupids);

$tempdir = make_temp_directory('threesixty/'.uniqid());
$pdfs = array();
foreach($records as $record) {
    $analysis = $DB->get_record('threesixty_analysis', array('activityid' => $threesixty->id, 'userid' => $record->id));
    if (!$analysis) {
        continue;
    }
    ...
    $pdf->footer_fullname = fullname($record);
    ...
    $pdf->setY(50);
    $title = '<p align="center">Influencing Spectrum Results<br />For<br />'.fullname($record).'</p>';
    $pdf->writeHTML($title);
    ...
    $filename = clean_filename(str_replace(' ', '_', $record->firstname . ' ' . $record->lastname . ' ' . $record->id . '.pdf'));
    $pdfs[] = $filename;
    $pdf->Output($tempdir . '/' . $filename, 'F');
}

$zipfile = 'threesixty_reports.zip';
$ziparch = new zip_archive();
if ($ziparch->open($tempdir.'/'.$zipfile, file_archive::OVERWRITE)) {
    foreach ($pdfs as $pdf) {
        $ziparch->add_file_from_pathname($pdf, $tempdir . '/' . $pdf);
    }
    $ziparch->close();
    if (!headers_sent()) {
        send_file($tempdir.'/'.$zipfile, $zipfile, 0, 0, false, true);
    }
}