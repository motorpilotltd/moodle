...
/* BEGIN CORE MOD */
    ...
    setlocale(LC_MONETARY, get_string($stringtofetch, 'langconfig'));
    $localeinfo = localeconv();
    if ($CFG->ostype == 'WINDOWS') {
        // Convert from Windows-1252 to UTF-8.
        $localeinfo['currency_symbol'] = iconv('Windows-1252', 'UTF-8', $localeinfo['currency_symbol']);
    }
    $symbol = htmlentities($localeinfo['currency_symbol']);
/* END CORE MOD */
    if (empty($symbol)) {
 ...
 */
/* BEGIN CORE MOD */
function facetoface_delete_session($session, $facetoface, $course, $cm) {
/* END CORE MOD */
    global $CFG, $DB;
...
/* BEGIN CORE MOD */
            if (facetoface_user_cancel($session, $facetoface, $course, $cm, $user->userid, true)) {
/* END CORE MOD */
...
/* BEGIN CORE MOD */
    ...
    $msg = str_replace(get_string('placeholder:sessioninfolink', 'facetoface'), $CFG->wwwroot.'/mod/facetoface/signup.php?s='.$sessionid, $msg);
/* END CORE MOD */
...
/* BEGIN CORE MOD */
            $from = facetoface_user::get_dummy_facetoface_user($fromaddress);
/* END CORE MOD */
        } else {
/* BEGIN CORE MOD */
            $from = get_admin();
...
/* BEGIN CORE MOD */
    ...
    if ($cm && $completion->is_enabled($cm) && $facetoface->completionbooked) {
        if ($newstatus >= MDL_F2F_STATUS_BOOKED) {
 ...
 */
/* BEGIN CORE MOD */
function facetoface_user_cancel($session, $facetoface, $course, $cm, $userid=false, $forcecancel=false, &$errorstr=null, $cancelreason='') {
    global $DB;
/* END CORE MOD */
    global $USER;
...
/* BEGIN CORE MOD */
        $completion = new completion_info($course);
        if ($completion->is_enabled($cm) && ($facetoface->completionbooked || $facetoface->completionattended)) {
            $completion->update_state($cm, COMPLETION_INCOMPLETE, $userid);
            $ccompletion = new completion_completion(array('course' => $course->id, 'userid' => $userid));
            $ccompletion->timecompleted = null;
            $ccompletion->mark_inprogress();
...
/* BEGIN CORE MOD */
                ...
                $body = facetoface_email_substitutions($posttext, $facetoface->name, $facetoface->reminderperiod,
                                                       $user, $session, $session->id);
                $filename = facetoface_get_ical_attachment($notificationtype, $facetoface, $session, $user, $body);
/* END CORE MOD */
                $htmlbody = ''; // TODO.
 ...
 */
/* BEGIN CORE MOD */
function facetoface_take_attendance($data, $completion, $cm) {
/* END CORE MOD */
    global $USER;
...
/* BEGIN CORE MOD */
            if (!facetoface_take_individual_attendance($submissionid, $grade, $completion, $cm)) {
/* END CORE MOD */
 ...
 */
/* BEGIN CORE MOD */
function facetoface_take_individual_attendance($submissionid, $grading, $completion, $cm) {
/* END CORE MOD */
    global $USER, $CFG, $DB;
...
/* BEGIN CORE MOD */
    if ($completion->is_enabled($cm) && $record->completionattended) {
        if ($grading == 100) {
            ...
            $ccompletion = new completion_completion(array('course' => $completion->course_id, 'userid' => $record->userid));
            $ccompletion->timecompleted = null;
            $ccompletion->mark_inprogress();
            $criteria = $completion->get_criteria();
            foreach($criteria as $criterion) {
...
/* BEGIN CORE MOD */
    // Call custom version of this function.
    facetoface_cm_info_view_custom($coursemodule);
    return null;
/* END CORE MOD */
    global $USER, $DB;
    $output = '';
 ...
 */
/* BEGIN CORE MOD */
function facetoface_get_ical_attachment($method, $facetoface, $session, $user, $description = '') {
/* END CORE MOD */
    global $CFG, $DB;
...
/* BEGIN CORE MOD */
        $description = facetoface_ical_escape(($description ? str_replace(array("\r", "\n"), '', nl2br($description)) : $session->details), true);
/* END CORE MOD */
...
/* BEGIN CORE MOD */
// Change timezone to Europe/London (Same as server).
    $template = <<<EOF
BEGIN:VCALENDAR
CALSCALE:GREGORIAN
...
/* BEGIN CORE MOD */
    // Two spaces post linebreak when wrapping.
...
function facetoface_get_completion_state($course, $cm, $userid, $type) {
    ...
    // Get facetoface details.
    $facetoface = $DB->get_record('facetoface', array('id'=>$cm->instance), '*',
            MUST_EXIST);

    $result = $type;

    if($facetoface->completionbooked) {
        // Need to show cancellations to get completions!?!
        $submissions = facetoface_get_user_submissions($facetoface->id, $userid, true);
        if ($submissions) {
            $status = end($submissions)->statuscode;
            // Completion if last status update shows booked or greater.
            $value = ($status >= MDL_F2F_STATUS_BOOKED);
        } else {
            // No submission so can't have completed.
            $value = false;
        }
    ...
    if($facetoface->completionattended) {
        // Need to show cancellations to get completions!?!
        $submissions = facetoface_get_user_submissions($facetoface->id, $userid, true);
        if ($submissions) {
            $status = end($submissions)->statuscode;
            // Completion if last status update shows fully attended (max status).
            $value = ($status == MDL_F2F_STATUS_FULLY_ATTENDED);
        } else {
            // No submission so can't have completed.
            $value = false;
        }
...
/**
 * Dummy facebook user class .
 */
class facetoface_user extends \core_user {
    public static function get_dummy_facetoface_user($email = '', $firstname = '', $lastname = '') {
...
/**
 * Customised *_cm_info_view() function.
 *
 * @param cm_info $cm
 * @return void
 */
function facetoface_cm_info_view_custom(cm_info $cm) {
    global $CFG, $USER, $DB, $OUTPUT;
    ...
    $htmlviewallsessions = html_writer::link(
        ...
        $strviewallsessions,
        array('title' => $strviewallsessions)
    );
    ...
    if ($submissions = facetoface_get_user_submissions($facetoface->id, $USER->id)) {
        ...
        if ($session = facetoface_get_session($submission->sessionid)) {
            ...
            $customfielddata = facetoface_get_customfielddata($session->id);
            $customfields = array();
            foreach ($customfielddata as $fielddata) {
                $customfields[$fielddata->shortname] = new stdClass();
                $customfields[$fielddata->shortname]->name = $fielddata->name;
                $customfields[$fielddata->shortname]->data = $fielddata->data;
            }

            $actions = array();
            $strmoreinfo = get_string('moreinfo', 'facetoface');
            $actions[] = html_writer::link(new moodle_url('/mod/facetoface/signup.php', array('s' => $session->id)), $strmoreinfo, array('title' => $strmoreinfo));
            // Don't include the link to view attendees if user is lacking capability.
            if (has_capability('mod/facetoface:viewattendees', context_course::instance($cm->course))) {
                ...
                $actions[] = html_writer::link(new moodle_url('/mod/facetoface/attendees.php', array('s' => $session->id)), $strattendees, array('title' => $strattendees));
            }
            // Don't include the link to cancel a session if it has already occurred.
            if (!facetoface_has_session_started($session, $timenow)) {
                $strcancelbooking = get_string('cancelbooking', 'facetoface');
                $actions[] = html_writer::link(new moodle_url('/mod/facetoface/cancelsignup.php', array('s' => $session->id)), $strcancelbooking, array('title' => $strcancelbooking));
            }

            // Add table data.
            $head = array(get_string('date', 'facetoface'), get_string('time', 'facetoface'), get_string('duration', 'facetoface'));
            $data = array($sessiondate, $sessiontime, format_duration($session->duration));
            foreach ($customfields as $customfield) {
                $head[] = $customfield->name;
                $data[] = $customfield->data;
            }
            $head[] = get_string('actions');
            $data[] = implode('<br />', $actions);
        ...
        $customfields = array();
        $customfielddetails = $DB->get_records('facetoface_session_field', array(), 'name', 'shortname, name');
        foreach ($customfielddetails as $fielddetails) {
            $customfields[$fielddetails->shortname] = new stdClass();
            $customfields[$fielddetails->shortname]->name = $fielddetails->name;
        }
        ...
        foreach ($sessions as $session) {
            ...
            $customfielddata = facetoface_get_customfielddata($session->id);
            $data = array($sessiondate, $sessiontime.$multiday, format_duration($session->duration));

            foreach ($customfields as $fieldname => $fielddata) {
                if (!empty($customfielddata[$fieldname])) {
                    $data[] = $customfielddata[$fieldname]->data;
                } else {
                    $data[] = '&nbsp;';
                }
            }

            $data[] = format_cost($session->normalcost);

            if ($cm->uservisible) {
                $data[] = html_writer::link(new moodle_url('/mod/facetoface/signup.php', array('s' => $session->id)), 'Book');
            }

            $table->data[] = $data;
        }
        if ($cm->uservisible && !empty($table->data)) {
            $cell = new html_table_cell();
            $cell->colspan = count($table->head);
            $cell->text = $htmlviewallsessions;
            $row = new html_table_row();
        ...
        $return = '';
        if ($cm->uservisible && !empty($table->data)) {
            $return .= html_writer::tag('p', get_string('signupforsession', 'facetoface').':');
        }
        if (!empty($table->data)) {
            ...
            $return .= get_string('noupcoming', 'facetoface');
        }

        $cm->set_after_link($return);
        return;
    } elseif (has_capability('mod/facetoface:viewemptyactivities', $contextmodule)) {
        $cm->set_after_link(
            html_writer::tag(
