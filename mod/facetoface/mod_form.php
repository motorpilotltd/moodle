...
/* BEGIN CORE MOD */
    public function get_data() {
        ...
        if ($data) {
            ...
            if (empty($data->completionbooked) || !$autocompletion || !empty($data->completionattended)) {
    ...
    public function validation($data, $files) {
        ...
        if (!empty($data['thirdparty'])) {
            $recipients = explode(',', $data['thirdparty']);
            foreach ($recipients as $recipient) {
                $email = trim($recipient);
                if (!validate_email($email)) {
                    $errors['thirdparty'] = get_string('err_email', 'form');
                }
    ...
    public function add_completion_rules() {
        ...
        $mform->addElement('checkbox',
                           'completionbooked',
                           '',
                           get_string('completionbooked', 'facetoface'));
        $mform->disabledIf('completionbooked', 'completionattended', 'checked');

        $mform->addElement('checkbox',
                           'completionattended',
                           '',
                           get_string('completionattended', 'facetoface'));
        $mform->disabledIf('completionattended', 'completionbooked', 'checked');

        return array('completionbooked', 'completionattended');
    }
