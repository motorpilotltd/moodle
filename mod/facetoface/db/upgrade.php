...
/* BEGIN CORE MOD */
    if ($oldversion < 2018070501) {

        // Remove all facetoface in events table
        $sql = "DELETE FROM {event}
                WHERE eventtype LIKE 'facetoface%'
                    OR modulename = 'facetoface'";
        $DB->execute($sql);

        // Update all f2f entry usercalentry/showoncalendar fields
        $sql = "UPDATE {facetoface}
                SET usercalentry = '0', showoncalendar = '0'
                WHERE usercalentry != '0' OR showoncalendar != '0'";

        $DB->execute($sql);
        // Facetoface savepoint reached.
        upgrade_mod_savepoint(true, 2018070501, 'facetoface');
    }
/* END CORE MOD */
    return $result;
}
