...
/* BEGIN CORE MOD */
        $mform->addElement('text', 'duration', get_string('duration', 'facetoface').get_string('durationcode', 'facetoface'), 'size="5"');
/* END CORE MOD */
...
/* BEGIN CORE MOD */
        if (!empty($data['duration'])) {
            ...
            if (count($time) > 2) {
                $errors['duration'] = get_string('validation:durationformatincorrect', 'facetoface').get_string('durationcode', 'facetoface');
            } elseif (isset($time[1]) && $time[1] != 0 && $time[1] != 30) {
                $errors['duration'] = get_string('validation:durationnothourorhalf', 'facetoface').get_string('durationcode', 'facetoface');
            }
        }
/* END CORE MOD */
        return $errors;
