...
/* BEGIN CORE MOD */
    $questionnaire->page->add_to_page('continue', (new moodle_url('/course/view.php', array('id' => $course->id)))->out(false));
/* END CORE MOD */
...
if ($questionnaire->qtype == QUESTIONNAIREONCE
    ...
    && empty($incompleteresponses)
    && count($completeresponses) == 1) {
    $reseturl = new moodle_url('/mod/questionnaire/reset.php', array('id' => $cm->id, 'rid' => reset($completeresponses)->id, 'self' => 1));
    echo $OUTPUT->box_start('generalbox boxaligncenter boxwidthwide');
    echo html_writer::link($reseturl, get_string('resetownresponse', 'questionnaire'));
    echo $OUTPUT->box_end();
}
/* END CORE MOD */
