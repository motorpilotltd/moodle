...
if ($questionnaire->capabilities->manage
        && $questionnaire->qtype == QUESTIONNAIREONCE
        && $questionnaire->respondenttype != 'anonymous'
        && $questionnaire->is_open()
        && !$questionnaire->is_closed()) {
    $reseturl = new moodle_url('/mod/questionnaire/reset.php', array('id' => $questionnaire->cm->id));
    $row[] = new tabobject('reset', $reseturl->out(), get_string('resetresponses', 'questionnaire'));
}
/* END CORE MOD */
