...
/* BEGIN CORE MOD */
    protected function process_questionnaire_response_hash($data) {
        global $DB;

        $data = (object)$data;
        $oldid = $data->id;
        $data->rid = $this->get_new_parentid('questionnaire_response');
        $data->qid = $this->get_mappingid('questionnaire_question', $data->qid);

        // Insert the questionnaire_response_text record.
        $newitemid = $DB->insert_record('questionnaire_resp_hashes', $data);
    }
/* END CORE MOD */
