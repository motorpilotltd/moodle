...
/* BEGIN CORE MOD */
        $questionnaire = new backup_nested_element('questionnaire', array('id'), array(
            ...
            'closedate', 'resume', 'navigate', 'grade', 'sid', 'timemodified', 'completionsubmit', 'autonum', 'sendsummary', 'usercanreset'));
/* END CORE MOD */
        ...
        $responsetext = new backup_nested_element('response_text', array('id'), array(
            'response_id', 'question_id', 'response'));
/* BEGIN CORE MOD */
        $responsehashes = new backup_nested_element('response_hashes');

        $responsehash = new backup_nested_element('response_hash', array('id'), array(
            'rid', 'qid', 'hash'));
/* END CORE MOD */
        // Build the tree.
...
/* BEGIN CORE MOD */
        $response->add_child($responsehashes);
        $responsehashes->add_child($responsehash);
/* END CORE MOD */
        // Define sources.
...
/* BEGIN CORE MOD */
                $responsehash->set_source_table('questionnaire_resp_hashes', array('rid' => backup::VAR_PARENTID));
/* END CORE MOD */
            }
