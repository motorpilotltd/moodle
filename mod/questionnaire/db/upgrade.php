...
/* BEGIN CORE MOD */
    if ($oldversion < 2013122201) {
        ...
        $qtable = new xmldb_table('questionnaire');
        $field = new xmldb_field('sendsummary', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '0', 'autonum');
        if (!$dbman->field_exists($qtable, $field)) {
            $dbman->add_field($qtable, $field);
        }

        // Add 'questionnaire_resp_hashes' table
        $table = new xmldb_table('questionnaire_resp_hashes');
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE,  null, null);
        $table->add_field('rid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0', 'id');
        $table->add_field('qid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0', 'rid');
        $table->add_field('hash', XMLDB_TYPE_CHAR, '255', null, XMLDB_NOTNULL, null, '0', 'qid');

        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'), null, null);
        $table->add_key('rid', XMLDB_KEY_UNIQUE, array('rid'), null, null);
        $table->add_index('hash_idx', XMLDB_INDEX_UNIQUE, array('hash'));
...
/* END CORE MOD */
    ...
    if ($oldversion < 2013122202.01) { // Capture previously conflicting upgrades.
/* END CORE MOD */
        ...
        // Questionnaire savepoint reached.
/* BEGIN CORE MOD */
        upgrade_mod_savepoint(true, 2013122202.01, 'questionnaire');
/* END CORE MOD */
    }
