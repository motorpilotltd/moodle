...
class questionnaire {
    ...
    public function view() {
        ...
        if (!$this->is_active()) {
            ...
            .'</div>';
/* BEGIN CORE MOD */
            echo html_writer::link(new moodle_url('/course/view.php', array('id' => $this->course->id)), get_string("continue"));
/* END CORE MOD */
...
/* BEGIN CORE MOD */
                $this->response_send_summary($this->rid);
/* END CORE MOD */
...
/* BEGIN CORE MOD */
    private function response_send_email($rid, $userid = false) {
        ...
        $record = $DB->get_record('questionnaire_survey', array('id' => $this->survey->id));

        if(!$record || empty($record->email)) {
            return(false);
        }

        $strresponded = ucfirst(get_string('response', 'questionnaire'));

        if ($this->respondenttype != 'anonymous') {
            $printusername = fullname($USER);
        } else {
            $printusername = get_string('anonymous', 'questionnaire');
        }

        $info = new stdClass();
        $info->username = $printusername;
        $info->questionnaire = format_string($this->name, true);
        $info->url = $CFG->wwwroot.'/mod/questionnaire/report.php?action=vresp&amp;sid='.$this->survey->id.
            '&amp;rid='.$rid.'&amp;instance='.$this->id;

        $postsubject = $this->course->fullname.' -> '.$info->questionnaire.' -> '.$strresponded.': '.$info->username;
        $posttext = $this->get_email_text($info);
        $posthtml = $this->get_email_html($info);

        $emailaddresses = preg_split('/,|;/', $record->email);
        $return = true;
        foreach ($emailaddresses as $email) {
            $userto = questionnaire_user::get_dummy_questionnaire_user(trim($email));
    ...
    private function get_email_text($info) {
        $coursecontext = context_course::instance($this->course->id);
        $courseshortname = format_string($this->course->shortname, true, array('context' => $coursecontext));
        $posttext  = $courseshortname.' -> '.get_string('modulenameplural', 'questionnaire').' -> '.
                        $info->questionnaire."\n";
        $posttext .= '---------------------------------------------------------------------'."\n";
        $posttext .= get_string("emailtext", "questionnaire", $info)."\n";
        $posttext .= '---------------------------------------------------------------------'."\n";
        return $posttext;
    }

    private function get_email_html($info) {
        global $CFG;
        $coursecontext = context_course::instance($this->course->id);
        $courseshortname = format_string($this->course->shortname, true, array('context' => $coursecontext));
        $course_url = $CFG->wwwroot.'/course/view.php?id='.$this->course->id;
        $questionnaire_all_url = $CFG->wwwroot.'/mod/questionnaire/index.php?id='.$this->course->id;
        $questionnaire_url = $CFG->wwwroot.'/mod/questionnaire/view.php?id='.$this->cm->id;

        $posthtml = '<p><font face="sans-serif">'.
                '<a href="'.$course_url.'">'.$courseshortname.'</a> -> '.
                '<a href="'.$questionnaire_all_url.'">'.get_string('modulenameplural', 'questionnaire').'</a> -> '.
                '<a href="'.$questionnaire_url.'">'.$info->questionnaire.'</a></font></p>';
        $posthtml .= '<hr /><font face="sans-serif">';
        $posthtml .= '<p>'.get_string('emailhtml', 'questionnaire', $info).'</p>';
        $posthtml .= '</font><hr />';
        return $posthtml;
    }
/* END CORE MOD */
...
/* BEGIN CORE MOD */
    public function print_link($rid) {
        global $OUTPUT;
        $imageurl = new moodle_url('/mod/questionnaire/images/print.gif');
        $image = html_writer::empty_tag('img', array('src' => $imageurl->out(), 'alt' => 'Printer-friendly version'));
        $url = new moodle_url('/mod/questionnaire/print.php', array('qid' => $this->id, 'rid' => $rid, 'courseid' => $this->course->id, 'sec' => '1'));
        $linktitle = get_string('printtooltip', 'questionnaire');
        $options = array('menubar' => true, 'location' => false, 'scrollbars' => true,
                         'resizable' => true, 'height' => 600, 'width' => 800);
        $name = 'popup';
        $link = new moodle_url($url);
        $action = new popup_action('click', $link, $name, $options);
        $actionlink = $OUTPUT->action_link($link, $image, $action, array('title' => $linktitle));
        echo html_writer::tag('p', $actionlink);
    }

    private function response_send_summary($rid) {
        global $DB, $USER;
        ...
        $resphash = $DB->get_record('questionnaire_resp_hashes', array('rid' => $rid));
        if (!$resphash) {
            $resphash = new stdClass();
            $resphash->rid = $rid;
            $resphash->qid = $this->id;
            $resphash->hash = $this->generate_uniquehash($USER->email);
            while ($DB->get_record('questionnaire_resp_hashes', array('hash' => $resphash->hash), 'id')) {
                // Re hash!
                $resphash->hash = generate_uniquehash($USER->email);
            }
            $resphash->id = $DB->insert_record('questionnaire_resp_hashes', $resphash);
        }

        $info = new stdClass();
		$info->name = fullname($USER);
		$info->questionnaire = format_string($this->name, true);
        $url = new moodle_url('/mod/questionnaire/printout.php', array('r' => $resphash->hash));
		$info->url = $url->out(false);

		$emailsubject = get_string('email:summary:subject', 'questionnaire', $info->questionnaire);
		$emailhtml = get_string('email:summary:body', 'questionnaire', $info);
		...
		email_to_user($to, $from, $emailsubject, $emailtext, $emailhtml);
    }

    private function generate_uniquehash($email){
        $timestamp = time();
        $salt = mt_rand();
...
/* BEGIN CORE MOD */
class questionnaire_user extends \core_user {
    public static function get_dummy_questionnaire_user($email = '', $firstname = '', $lastname = '') {
        ...
        $user->username = 'questionnaireuser';
