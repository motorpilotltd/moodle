...
function xmldb_aruphonestybox_upgrade($oldversion) {
    ...
    if ($oldversion < 2017051500) {
        ...
        $updatesql = "UPDATE {local_taps_enrolment}
                   SET classtype = :classtype, classname = :classname, provider = :provider, providerid = :providerid, locked = :locked
                 WHERE id IN (
                           SELECT lte.id
        ...
        foreach ($instances as $instance) {
            ...
            $instance->provider = 'Learning Burst';
            $instance->classname = $course->fullname;
            $params = [
                'ahbid' => $instance->id,
                'classtype' => $instance->provider,
                'classname' => $instance->classname,
                'provider' => $instance->provider,
                'providerid' => $instance->course,
                'locked' => 1,
            ];
