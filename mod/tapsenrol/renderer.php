...
class mod_tapsenrol_renderer extends plugin_renderer_base {
    ...
    public function enrolment_history($tapsenrol, $enrolments, $classes, $cmid, $allclasses) {
        ...
        if ($enrolments || $classes) {
            $seatsremaining = $tapsenrol->taps->get_seats_remaining_by_course($tapsenrol->course->id);
        }
        ...
        if ($enrolments) {
            ...
            foreach ($enrolments as $enrolment) {
                ...
                if (!empty($class->durationunitscode) && $class->durationunitscode == 'H') {
                    $cells[] = $class->duration ? \mod_tapsenrol\taps::duration_hours_display($class->duration, $class->durationunits) : '-';
                } else {
        ...
        if ($classes) {
            foreach ($classes as $index => $class) {
                ...
                if (!empty($class->classdurationunitscode) && $class->classdurationunitscode == 'H') {
                    $cells[] = $class->classduration ? \mod_tapsenrol\taps::duration_hours_display($class->classduration, $class->classdurationunits) : '-';
                } else {
                    $cells[] = $class->classduration ? (float) $class->classduration.' '.$class->classdurationunits : '-';
                }
    ...
    public function review_enrolment($tapsenrol, $class) {
        ...
        if (!empty($class->classdurationunitscode) && $class->classdurationunitscode == 'H') {
            $durationvalue = isset($class->classduration) ? \mod_tapsenrol\taps::duration_hours_display($class->classduration, $class->classdurationunits) : get_string('tbc', 'tapsenrol');
        } else {
    ...
    public function cancel_enrolment($tapsenrol, $enrolment, $class) {
        ...
        $duration = html_writer::tag('div', get_string('duration', 'tapsenrol'). ':', array('class' => 'fitemtitle'));
        if (!empty($class->durationunitscode) && $class->durationunitscode == 'H') {
            $durationvalue = isset($class->duration) ? \mod_tapsenrol\taps::duration_hours_display($class->duration, $class->durationunits) : get_string('tbc', 'tapsenrol');
        } else {
            $durationvalue = isset($class->duration) ? (float) $class->duration . ' ' . $class->durationunits : get_string('tbc', 'tapsenrol');
        }
        $duration .= html_writer::tag('div', $durationvalue, array('class' => 'felement'));
        $html .= html_writer::tag('div', $duration, array('class' => 'fitem'));
...
EOF;
    ...
    public function admin_links($tapsenrol) {
        ...
        if (has_capability('mod/tapsenrol:createclass', $tapsenrol->context->course)) {
            ...
            $links[] = array(
                'url' => $editclassurl,
                'title' => get_string('manageclasses', 'tapsenrol')
            );
        }
