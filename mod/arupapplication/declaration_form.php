...
class declaration_form extends moodleform {
    public function definition() {
        global $DB;

        $mform =& $this->_form;
        $applicationid = $this->_customdata['applicationid'];
        $declarations = $DB->get_records('arupdeclarations', array('applicationid'=>$applicationid), 'sortorder');

        $mform->addElement('html', '<h2>' . get_string('heading:declaration', 'arupapplication') . '</h2>');

        foreach($declarations as $declaration) {
            $mform->addElement('advcheckbox', 'declarationid'.$declaration->id, '', $declaration->declaration, array('group' => 0), array(0, 1));
        }
        ...
        $mform->addElement('hidden', 'gopage', 4);
        $mform->setType('gopage', PARAM_INT);
        $mform->addElement('hidden', 'thispage', 'declarations');
